 ...
 */
static const ExaGOSelfcheckTCOPFLOWAnswer
    ExaGOSelfcheckTCOPFLOWAnswers[EXAGOSelfcheckTCOPFLOWNumAnswers] = {
        {SELFCHECK_NETWORK_CASE9_WIND, TCOPFLOW_INITIALIZATION,
         TCOPFLOW_GENBUSVOLTAGE, SELFCHECK_SCENARIO_CASE9, SELFCHECK_QLOAD,
         SELFCHECK_PLOAD, PETSC_FALSE, 5.0, 0.5, PETSC_FALSE, 12,
         2.0792822464162207e+04}};

PetscErrorCode
ExaGOSelfcheckTCOPFLOWFindAnswer(TCOPFLOW tcopflow,
  ...
  ierr = PetscOptionsGetString(NULL, NULL, "-netfile", netfile,
  ...
  ierr = PetscOptionsGetString(NULL, NULL, "-opflow_initialization",
                               ans->modelinit, PETSC_MAX_PATH_LEN, &flg);
  CHKERRQ(ierr);
  if (!flg) {
    ExaGOLog(EXAGO_LOG_WARN, "%s needs option -opflow_initialization.",
             __func__);
    return PETSC_TRUE;
  }

  // Check that a model parameter is being passed
  ierr = PetscOptionsGetString(NULL, NULL, "-opflow_genbusvoltage",
                               ans->genbusvoltage, PETSC_MAX_PATH_LEN, &flg);
  CHKERRQ(ierr);
  if (!flg) {
    ExaGOLog(EXAGO_LOG_WARN, "%s needs option -opflow_genbusvoltage.",
             __func__);
    return PETSC_TRUE;
  }

  // Check for windfile
  ierr = PetscOptionsGetString(NULL, NULL, "-tcopflow_windgenprofile", windfile,
                               PETSC_MAX_PATH_LEN, &flg);
  CHKERRQ(ierr);
  if (!flg) {
    ExaGOLog(EXAGO_LOG_WARN, "%s needs option -tcopflow_windgenprofile.",
             __func__);
    return PETSC_TRUE;
  }
  ...
  ierr = PetscOptionsGetString(NULL, NULL, "-tcopflow_ploadprofile", ploadfile,
                               PETSC_MAX_PATH_LEN, &flg);
  CHKERRQ(ierr);
  if (!flg) {
    ExaGOLog(EXAGO_LOG_WARN, "%s needs option -tcopflow_ploadprofile .",
             __func__);
    return PETSC_TRUE;
  }
  ...
  ierr = PetscOptionsGetString(NULL, NULL, "-tcopflow_qloadprofile", qloadfile,
                               PETSC_MAX_PATH_LEN, &flg);
  CHKERRQ(ierr);
  if (!flg) {
    ExaGOLog(EXAGO_LOG_WARN, "%s needs option -tcopflow_qloadprofile.",
             __func__);
    return PETSC_TRUE;
  }
  ...
  ierr = PetscOptionsGetBool(NULL, NULL, "-tcopflow_iscoupling",
                             &ans->iscoupling, &flg);
  CHKERRQ(ierr);
  if (!flg) {
  ...
  // Check for dt
  ierr = PetscOptionsGetReal(NULL, NULL, "-tcopflow_dT", &ans->dt, &flg);
  CHKERRQ(ierr);
  if (!flg) {
    ExaGOLog(EXAGO_LOG_WARN,
             "%s needs option -tcopflow_dT with -sopflow_enable_multiperiod.",
             __func__);
    return PETSC_TRUE;
  }

  // Check for duration
  ierr = PetscOptionsGetReal(NULL, NULL, "-tcopflow_duration", &ans->duration,
                             &flg);
  CHKERRQ(ierr);
  if (!flg) {
    ExaGOLog(
        EXAGO_LOG_WARN,
        "%s needs option -tcopflow_duration with -sopflow_enable_multiperiod.",
        __func__);
    return PETSC_TRUE;
  }

  // Check for opflow_ignore_lineflow_constraints
  ierr = PetscOptionsGetBool(NULL, NULL, "-opflow_ignore_lineflow_constraints",
                             &ans->lineflow_constraints, &flg);
  CHKERRQ(ierr);
  if (!flg) {
    ExaGOLog(EXAGO_LOG_WARN,
             "%s needs option -opflow_ignore_lineflow_constraints.", __func__);
    return PETSC_TRUE;
  }

  int i;
  for (int i = 0; i < EXAGOSelfcheckTCOPFLOWNumAnswers; i++) {
    const ExaGOSelfcheckTCOPFLOWAnswer *ians =
        &ExaGOSelfcheckTCOPFLOWAnswers[i];
  ...
  ExaGOLog(
      EXAGO_LOG_WARN,
      "%s could not find answer for option:"
      "\n\tnetwork:%s\n\twindgen:%s\n\tinitialization:%s\n\tgenbusvoltage:%s\
       \n\tiscoupling:%d\n\tlineflow_constraints:%d\
       \n\tpload:%s\n\tqload:%s\n\tdT:%f\n\tduration:%f\n",
      __func__, ans->networkname, ans->modelinit, ans->genbusvoltage,
      ans->windgenname, ans->iscoupling, ans->lineflow_constraints,
...
PetscErrorCode ExaGOSelfcheckTCOPFLOW(TCOPFLOW tcopflow) {
  ...
  ExaGOSelfcheckTCOPFLOWAnswer *ans;
  ans = (ExaGOSelfcheckTCOPFLOWAnswer *)malloc(
      sizeof(ExaGOSelfcheckTCOPFLOWAnswer));
  ...
  ierr = TCOPFLOWGetConvergenceStatus(tcopflow, &conv_status);
  CHKERRQ(ierr);
  if (conv_status == PETSC_FALSE) {
    fail++;
    ExaGOLog(EXAGO_LOG_ERROR,
             "ExaGO TCOPFLOW %s functionality test failed to converge.\n",
             config);
  }

  // Compare objective value
  ierr = TCOPFLOWGetObjective(tcopflow, &obj_value);
  CHKERRQ(ierr);
  ierr = TCOPFLOWGetTolerance(tcopflow, &tolerance);
  CHKERRQ(ierr);
  if (!IsEqual(obj_value, ans->objective, tolerance, error)) {
    fail++;
    ExaGOLog(EXAGO_LOG_ERROR,
             "ExaGO SOPFLOW %s functionality test failed to match expected "
             "objective value:\n"
             "%-30s%.12e\n"
             "%-30s%.12e\n"
             "%-30s%.12e\n"
             "%-30s%.12e\n",
             config, "Expected:", ans->objective, "Actual:", obj_value,
             "Tolerance:", tolerance, "Scaled Error:", error);
  }

  // Compare iterations
  ierr = TCOPFLOWGetNumIterations(tcopflow, &numiter);
  CHKERRQ(ierr);
  if (ans->numiter != numiter) {
    fail++;
    ExaGOLog(EXAGO_LOG_ERROR,
             "ExaGO TCOPFLOW %s functionality test failed to match expected "
