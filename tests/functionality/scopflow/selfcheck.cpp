  ...
  : public FunctionalityTestContext<ScopflowFunctionalityTestParameters>
{
  ...
  void run_test_case(Params& params) override
  {
    ...
    ierr = SCOPFLOWSetNetworkData(scopflow,params.network.c_str());ExaGOCheckError(ierr);
    ...
    ierr = SCOPFLOWSetContingencyData(scopflow, NATIVE,
        params.contingencies.c_str());ExaGOCheckError(ierr);

    // Prepend installation directory to network path
    resolve_datafiles_path(params.pload);
    ierr = SCOPFLOWSetPLoadData(scopflow,params.pload.c_str());ExaGOCheckError(ierr);

    // Prepend installation directory to network path
    resolve_datafiles_path(params.qload);
    ierr = SCOPFLOWSetQLoadData(scopflow,params.qload.c_str());ExaGOCheckError(ierr);

    // Prepend installation directory to network path
    resolve_datafiles_path(params.windgen);
    ierr = SCOPFLOWSetWindGenProfile(scopflow,params.windgen.c_str());ExaGOCheckError(ierr);

    // Set number of contingencies
    ierr = SCOPFLOWSetNumContingencies(scopflow, params.num_contingencies);ExaGOCheckError(ierr);

    ierr = SCOPFLOWEnableMultiPeriod(scopflow, (PetscBool)params.multiperiod);ExaGOCheckError(ierr);
    ierr = SCOPFLOWSetTimeStep(scopflow, params.dT);ExaGOCheckError(ierr);
    ierr = SCOPFLOWSetDuration(scopflow, params.duration);ExaGOCheckError(ierr);

    ierr = SCOPFLOWSetSolver(scopflow, params.solver.c_str());ExaGOCheckError(ierr);

    ierr = SCOPFLOWSetInitilizationType(scopflow,
        (OPFLOWInitializationType)params.opflow_initialization);ExaGOCheckError(ierr);
    ierr = SCOPFLOWSetGenBusVoltageType(scopflow,
        (OPFLOWGenBusVoltageType)params.opflow_genbusvoltage);ExaGOCheckError(ierr);

    ierr = SCOPFLOWSetModel(scopflow, params.model.c_str());ExaGOCheckError(ierr);

    ierr = SCOPFLOWSetMode(scopflow, (PetscInt)params.mode);ExaGOCheckError(ierr);

    ierr = SCOPFLOWSetUp(scopflow);ExaGOCheckError(ierr);

    ierr = SCOPFLOWSolve(scopflow);ExaGOCheckError(ierr);
    ...
    ierr = SCOPFLOWGetConvergenceStatus(scopflow,&params.conv_status);ExaGOCheckError(ierr);
    ...
    ierr = SCOPFLOWGetObjective(scopflow,&params.obj_value);ExaGOCheckError(ierr);
    ...
    /* Test num iterations */
    ierr = SCOPFLOWGetNumIterations(scopflow,&params.numiter);ExaGOCheckError(ierr);
    ...
    ierr = SCOPFLOWDestroy(&scopflow);ExaGOCheckError(ierr);
  }
};
