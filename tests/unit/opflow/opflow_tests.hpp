...
public:
  ...
  LocalOrdinalType computeObjective(OPFLOW opflow, Vec X, RealType obj_ref) {
    ...
    RealType obj_val;
    ierr = OPFLOWComputeObjective(opflow, X, &obj_val);
    CHKERRQ(ierr);
  ...
  LocalOrdinalType computeObjective(OPFLOW opflow, double *xref,
                                    RealType obj_ref) {
    PetscErrorCode ierr;
    LocalOrdinalType fail = 0;
    RealType obj_val = 0.0;
    ierr = OPFLOWComputeObjectiveArray(opflow, xref, &obj_val);
    CHKERRQ(ierr);
    fail += !isEqual(obj_val, obj_ref);
    cleanup(fail, opflow);
  }
  ...
  LocalOrdinalType computeGradient(OPFLOW opflow, Vec X, Vec grad_ref) {
    ...
    ierr = VecDuplicate(grad_ref, &grad);
    CHKERRQ(ierr);
  ...
  LocalOrdinalType computeGradient(OPFLOW opflow, double *xref,
                                   double *grad_ref) {
    PetscErrorCode ierr;
    double *grad;
    LocalOrdinalType fail = 0;
    ...
    ierr = PetscMalloc1(nx, &grad);
    CHKERRQ(ierr);

    ierr = OPFLOWComputeGradientArray(opflow, xref, grad);
    CHKERRQ(ierr);
...
#if defined(EXAGO_ENABLE_RAJA)
  LocalOrdinalType computeGradient(OPFLOW opflow, double *xref_dev,
                                   double *grad_ref,
    ...
    ierr = OPFLOWComputeGradientArray(opflow, xref_dev, grad_dev);
    CHKERRQ(ierr);
  ...
  LocalOrdinalType computeVariableBounds(OPFLOW opflow, Vec Xlref, Vec Xuref) {
    ...
    ierr = VecDuplicate(Xlref, &Xl);
    CHKERRQ(ierr);
    ierr = VecDuplicate(Xuref, &Xu);
    CHKERRQ(ierr);
    ierr = OPFLOWComputeVariableBounds(opflow, Xl, Xu);
    CHKERRQ(ierr);
  ...
  LocalOrdinalType computeVariableBounds(OPFLOW opflow, double *xlref,
                                         double *xuref) {
    PetscErrorCode ierr;
    LocalOrdinalType fail = 0;
    double *xu, *xl;
    ...
    ierr = PetscMalloc1(nx, &xu);
    CHKERRQ(ierr);
    ierr = PetscMalloc1(nx, &xl);
    CHKERRQ(ierr);
  ...
  LocalOrdinalType computeVariableBounds(OPFLOW opflow, double *xlref,
    ...
    xu_dev = xu;
#endif
    ierr = (*opflow->modelops.setvariableboundsarray)(opflow, xl_dev, xu_dev);
    CHKERRQ(ierr);
  ...
  LocalOrdinalType computeConstraints(OPFLOW opflow, double *xref,
                                      double *gref) {
    PetscErrorCode ierr;
    LocalOrdinalType fail = 0;
    double *g, *ge, *gi;
    ...
    ierr = OPFLOWComputeEqualityConstraintsArray(opflow, xref, ge);
    CHKERRQ(ierr);

    if (opflow->nconineq) {
      ierr = OPFLOWComputeInequalityConstraintsArray(opflow, xref, gi);
      CHKERRQ(ierr);
    }
...
#if defined(EXAGO_ENABLE_RAJA)
  LocalOrdinalType computeConstraints(OPFLOW opflow, double *xref_dev,
                                      double *gref,
                                      umpire::ResourceManager &resmgr) {
    PetscErrorCode ierr;
    LocalOrdinalType fail = 0;
    ...
    g = static_cast<double *>(
        h_allocator.allocate(opflow->ncon * sizeof(double)));
#ifdef EXAGO_ENABLE_GPU
    umpire::Allocator d_allocator = resmgr.getAllocator("DEVICE");
    g_dev = static_cast<double *>(
        d_allocator.allocate(opflow->ncon * sizeof(double)));
#else
    g_dev = g;
#endif
    ...
    ierr = OPFLOWComputeEqualityConstraintsArray(opflow, xref_dev, ge_dev);
    CHKERRQ(ierr);

    if (opflow->nconineq) {
      ierr = OPFLOWComputeInequalityConstraintsArray(opflow, xref_dev, gi_dev);
      CHKERRQ(ierr);
    }
  ...
  LocalOrdinalType computeConstraintBounds(OPFLOW opflow, Vec Glref,
                                           Vec Guref) {
    PetscErrorCode ierr;
    LocalOrdinalType fail = 0;
    Vec Gl, Gu;

    ierr = VecDuplicate(Glref, &Gl);
    CHKERRQ(ierr);
    ierr = VecDuplicate(Guref, &Gu);
    CHKERRQ(ierr);
    ierr = OPFLOWComputeConstraintBounds(opflow, Gl, Gu);
    CHKERRQ(ierr);
  ...
  LocalOrdinalType computeConstraintBounds(OPFLOW opflow, double *glref,
                                           double *guref) {
    PetscErrorCode ierr;
    LocalOrdinalType fail = 0;
    double *gl, *gu;
  ...
  LocalOrdinalType computeConstraintBounds(OPFLOW opflow, double *glref,
    ...
    gl = static_cast<double *>(
        h_allocator.allocate(opflow->ncon * sizeof(double)));
    gu = static_cast<double *>(
        h_allocator.allocate(opflow->ncon * sizeof(double)));

#ifdef EXAGO_ENABLE_GPU
    umpire::Allocator d_allocator = resmgr.getAllocator("DEVICE");
    gl_dev = static_cast<double *>(
        d_allocator.allocate(opflow->ncon * sizeof(double)));
    gu_dev = static_cast<double *>(
        d_allocator.allocate(opflow->ncon * sizeof(double)));
#else
    gl_dev = gl;
    gu_dev = gu;
    ...
    ierr = (*opflow->modelops.setconstraintboundsarray)(opflow, gl_dev, gu_dev);
    CHKERRQ(ierr);
  ...
  LocalOrdinalType computeConstraintJacobian(OPFLOW opflow, Vec X, Mat Jeqref,
                                             Mat Jineqref) {
    PetscErrorCode ierr;
    LocalOrdinalType fail = 0;
    Mat Jeq, Jineq;
    ...
    ierr = MatMult(Jeq, X, temp1);
    CHKERRQ(ierr);
    ierr = MatMult(Jeqref, X, temp2);
    CHKERRQ(ierr);
  ...
  LocalOrdinalType computeConstraintJacobian(OPFLOW opflow, double *x,
    ...
    // Re-order the reference solution using IS (Index sets), and then slice for
    // the sparse and dense components
    ierr = OPFLOWGetVariableOrdering(opflow, &idxn2sd_map);
    CHKERRQ(ierr);
    ierr = MatGetSize(Jeqref_nat, &nrow, &ncol);
    CHKERRQ(ierr);
    ...
    ierr = ISCreateGeneral(PETSC_COMM_WORLD, nrow, row_map, PETSC_COPY_VALUES,
                           &row_map_IS);
    CHKERRQ(ierr);
    ierr = ISCreateGeneral(PETSC_COMM_WORLD, ncol, col_map, PETSC_COPY_VALUES,
                           &col_map_IS);
    CHKERRQ(ierr);
    ierr = ISCreateGeneral(PETSC_COMM_WORLD, nxsparse, spcol_map,
                           PETSC_COPY_VALUES, &spcol_map_IS);
    CHKERRQ(ierr);
    ierr = ISCreateGeneral(PETSC_COMM_WORLD, nxdense, dncol_map,
                           PETSC_COPY_VALUES, &dncol_map_IS);
    CHKERRQ(ierr);

    ierr = MatPermute(Jeqref_nat, row_map_IS, col_map_IS, &Jeqref_spdn);
    CHKERRQ(ierr);

    ierr = MatCreateSubMatrix(Jeqref_spdn, row_map_IS, spcol_map_IS,
                              MAT_INITIAL_MATRIX, &Jeqref_sparse);
    CHKERRQ(ierr);
    ierr = MatCreateSubMatrix(Jeqref_spdn, row_map_IS, dncol_map_IS,
                              MAT_INITIAL_MATRIX, &Jeqref_dense);
    CHKERRQ(ierr);
    ...
    ierr = PetscMalloc1(nnz, &iRow);
    CHKERRQ(ierr);
    ierr = PetscMalloc1(nnz, &jCol);
    CHKERRQ(ierr);
    ierr = PetscMalloc1(nnz, &values);
    CHKERRQ(ierr);
    ...
    ierr = PetscMalloc1(nrow * nxdense, &Jeq_dense);
    CHKERRQ(ierr);
    ...
    if (opflow->Nconineq) {
      // Since the inequality contraints have a different number of rows, we
      // must re-create some things first
      IS ineq_row_map_IS;
      int *ineq_row_map;
      PetscInt nrow_ineq, ncol_ineq;

      ierr = MatGetSize(Jineqref_nat, &nrow_ineq, &ncol_ineq);
      CHKERRQ(ierr);
      ...
      ierr = ISCreateGeneral(PETSC_COMM_WORLD, nrow_ineq, ineq_row_map,
                             PETSC_COPY_VALUES, &ineq_row_map_IS);
      CHKERRQ(ierr);

      // Re-order and get dense components
      Mat Jineqref_spdn, Jineqref_dense;
      ierr =
          MatPermute(Jineqref_nat, ineq_row_map_IS, col_map_IS, &Jineqref_spdn);
      CHKERRQ(ierr);
      ierr = MatCreateSubMatrix(Jineqref_spdn, ineq_row_map_IS, dncol_map_IS,
                                MAT_INITIAL_MATRIX, &Jineqref_dense);
      CHKERRQ(ierr);

      // Get the hiop dense matrix solution
      double *Jineq_dense;

      ierr = PetscMalloc1(nrow_ineq * nxdense, &Jineq_dense);
      CHKERRQ(ierr);

      ierr = (*opflow->modelops.computedenseinequalityconstraintjacobianhiop)(
          opflow, x, Jineq_dense);
      CHKERRQ(ierr);
      ...
      // Cleanup
      delete[] ineq_row_map;
      ierr = MatDestroy(&Jineqref_spdn);
      CHKERRQ(ierr);
      ierr = MatDestroy(&Jineqref_dense);
      CHKERRQ(ierr);

      ierr = PetscFree(Jineq_dense);
      CHKERRQ(ierr);
    }

    // Cleanup
    ierr = MatDestroy(&Jeqref_spdn);
    CHKERRQ(ierr);
    ierr = MatDestroy(&Jeqref_sparse);
    CHKERRQ(ierr);
    ierr = MatDestroy(&Jeqref_dense);
    CHKERRQ(ierr);

    ierr = ISDestroy(&col_map_IS);
    CHKERRQ(ierr);
    ierr = ISDestroy(&row_map_IS);
    CHKERRQ(ierr);
    ierr = ISDestroy(&spcol_map_IS);
    CHKERRQ(ierr);
    ierr = ISDestroy(&dncol_map_IS);
    CHKERRQ(ierr);
  ...
  LocalOrdinalType computeConstraintJacobian(OPFLOW opflow, double *x_dev,
    ...
    Jeq_dense = static_cast<double *>(
        h_allocator.allocate(nrow * nxdense * sizeof(double *)));
#ifdef EXAGO_ENABLE_GPU
    Jeq_dense_dev = static_cast<double *>(
        d_allocator.allocate(nrow * nxdense * sizeof(double *)));
#else
    Jeq_dense_dev = Jeq_dense;
#endif

    ierr = (*opflow->modelops.computedenseequalityconstraintjacobianhiop)(
        opflow, x_dev, Jeq_dense_dev);
    CHKERRQ(ierr);
    ...
    if (opflow->Nconineq) {
      ...
      Jineq_dense = static_cast<double *>(
          h_allocator.allocate(nxdense * nrow_ineq * sizeof(double *)));
#ifdef EXAGO_ENABLE_GPU
      Jineq_dense_dev = static_cast<double *>(
          d_allocator.allocate(nxdense * nrow_ineq * sizeof(double *)));
#else
      Jineq_dense_dev = Jineq_dense;
#endif

      ierr = (*opflow->modelops.computedenseinequalityconstraintjacobianhiop)(
          opflow, x_dev, Jineq_dense_dev);
      CHKERRQ(ierr);
  ...
  LocalOrdinalType computeHessian(OPFLOW opflow, Vec X, Vec Lambda,
    ...
    ierr = MatDuplicate(Hessref, MAT_DO_NOT_COPY_VALUES, &Hess);
    CHKERRQ(ierr);

    ierr = OPFLOWComputeHessian(opflow, X, Lambda, obj_factor, Hess);
    CHKERRQ(ierr);

    ierr = VecDuplicate(X, &temp1);
    CHKERRQ(ierr);
    ierr = VecDuplicate(temp1, &temp2);
    CHKERRQ(ierr);
    ...
    ierr = MatMult(Hess, X, temp1);
    CHKERRQ(ierr);
    ierr = MatMult(Hessref, X, temp2);
    CHKERRQ(ierr);
   ...
   */
  LocalOrdinalType computeHessian(OPFLOW opflow, double *x_ref,
                                  double *lambda_ref, PetscScalar obj_factor,
                                  Mat Hessref) {
    PetscErrorCode ierr;
    LocalOrdinalType fail = 0;
    PetscInt nrow, ncol;
    ...
    ierr = OPFLOWGetVariableOrdering(opflow, &idxn2sd_map);
    CHKERRQ(ierr);
    ierr = MatGetSize(Hessref, &nrow, &ncol);
    CHKERRQ(ierr);
    ...
    ierr = ISCreateGeneral(PETSC_COMM_WORLD, nrow, permutation_map,
                           PETSC_COPY_VALUES, &permutation_map_IS);
    CHKERRQ(ierr);
    ierr = ISCreateGeneral(PETSC_COMM_WORLD, nxsparse, spcol_map,
                           PETSC_COPY_VALUES, &spcol_map_IS);
    CHKERRQ(ierr);
    ...
    ierr = MatPermute(Hessref, permutation_map_IS, permutation_map_IS,
                      &Hessref_spdn);
    CHKERRQ(ierr);

    ierr = MatCreateSubMatrix(Hessref_spdn, spcol_map_IS, spcol_map_IS,
                              MAT_INITIAL_MATRIX, &Hessref_sparse);
    CHKERRQ(ierr);
    ierr = MatCreateSubMatrix(Hessref_spdn, dncol_map_IS, dncol_map_IS,
                              MAT_INITIAL_MATRIX, &Hessref_dense);
    CHKERRQ(ierr);

    // Test the dense component
    double *hess_dense;
    hess_dense = new double[nxdense * nxdense];
    ierr = (*opflow->modelops.computedensehessianhiop)(opflow, x_ref,
                                                       lambda_ref, hess_dense);
    CHKERRQ(ierr);
    ...
    ierr = (*opflow->modelops.computesparsehessianhiop)(
        opflow, x_ref, lambda_ref, iRow, jCol, values);
    CHKERRQ(ierr);
    ...
    delete[] jCol;
    delete[] values;
    ierr = MatDestroy(&Hessref_spdn);
    CHKERRQ(ierr);
    ierr = MatDestroy(&Hessref_sparse);
    CHKERRQ(ierr);
    ierr = MatDestroy(&Hessref_dense);
    CHKERRQ(ierr);
    ierr = ISDestroy(&permutation_map_IS);
    CHKERRQ(ierr);
    ierr = ISDestroy(&spcol_map_IS);
    CHKERRQ(ierr);
    ierr = ISDestroy(&dncol_map_IS);
...
private:
  ...
  LocalOrdinalType reduceReturn(LocalOrdinalType fail, MPI_Comm comm) const {
    ...
    int ierr = MPI_Allreduce(&fail, &gFail, 1, MPI_INT, MPI_SUM, comm);
    assert(ierr == 0);
    return static_cast<LocalOrdinalType>(gFail != 0);
  }

  virtual LocalOrdinalType reduceReturn(LocalOrdinalType fail,
                                        OPFLOW opflow) const {
    if (opflow == nullptr)
      THROW_NULL_DEREF;
   ...
   *
   */
  virtual int verifyAnswer(const double *avals, const double *bvals, int size,
                           const RealType &tol = eps) const {
    LocalOrdinalType fail = 0;

    for (int i = 0; i < size; i++) {
      if (!isEqual(avals[i], bvals[i], tol)) {
        printf("avals[%d] = %18.16f not equal to bvals[%d] = %18.16f. Exceeds "
               "set tolerance = %g\n",
  ...
  virtual int verifyAnswer(Vec a, Vec b, const RealType &tol = eps) const {
    ...
    ierr = VecGetSize(a, &size);
    CHKERRQ(ierr);
    ierr = VecGetArrayRead(a, &avals);
    CHKERRQ(ierr);
    ierr = VecGetArrayRead(b, &bvals);
    CHKERRQ(ierr);

    fail = verifyAnswer(avals, bvals, size, tol);

    ierr = VecRestoreArrayRead(a, &avals);
    CHKERRQ(ierr);
    ierr = VecRestoreArrayRead(b, &bvals);
    CHKERRQ(ierr);
    return reduceReturn(fail, (PetscObject)a);
  }
  ...
  /**
   * @brief Compare two matrices. Matrix a is in PETSc sparse matrix format and
   * b is a dense matrix.
  ...
  /**
   * @brief Compares two matrices - a is in PETSc format and the other matrix is
   * described by sparse triplet iRow, jCol, values
   *
   */
  virtual int verifyAnswer(Mat a, int nnz, int *iRow, int *jCol, double *values,
                           const RealType &tol = eps) const {
    double val;
    int fail = 0;
    PetscErrorCode ierr;
    ...
    for (int i = 0; i < nnz; i++) {
      ierr = MatGetValues(a, 1, iRow + i, 1, jCol + i, &val);
      CHKERRQ(ierr);

      if (!isEqual(val, values[i], tol)) {
        std::cout << "Failed for index (" << iRow[i] << ", " << jCol[i]
