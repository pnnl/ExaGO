...
/*
  ...
  We are using a macro to avoid changing the error message in many places should
  the need arise.
*/
...
protected:
  ...
  static void printMessage(const LocalOrdinalType fail, const char *funcname,
                           const LocalOrdinalType rank) {
    if (fail > 0) {
      std::cout << RED << "--- FAIL: Test " << funcname << " on rank " << rank
                << CLEAR << "\n";
