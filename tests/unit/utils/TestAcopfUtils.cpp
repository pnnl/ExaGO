...
void readFromFile(Vec* vector, std::string file)
{
  ...
  if(f.is_open())
  {
    ...
    ierr = VecSetSizes(*vector, n, PETSC_DECIDE);CHKERRV(ierr);
    ...
    ierr = VecSetValues(*vector, n, indices, values, INSERT_VALUES);CHKERRV(ierr);
...
bool readFromFile(Mat *a, std::string file_name)
{
  ...
  if (FILE *file = fopen(file_name.c_str(), "r")) 
  {
    fclose(file);
    PetscViewer viewer;
    PetscViewerBinaryOpen(PETSC_COMM_WORLD, file_name.c_str(), FILE_MODE_READ, &viewer);
...
void validate_directory(std::string pth)
{
  ...
  if(dp == NULL)
  {
    ...
    int status = mkdir(pth.c_str(), S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH); // Permissions to read/write/search for group and read/search for others
    if(status != 0)
    {
