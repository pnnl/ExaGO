/**
 * @file tcopflowimpl.h
 * @brief Private header file that defines data and structures for multi-period
...
struct _p_TCOPFLOWModelOps {
  ...
  PetscErrorCode (*computeobjandgradient)(
      TCOPFLOW, Vec, PetscScalar *, Vec); /* Objective and gradient routine */
  PetscErrorCode (*computeobjective)(TCOPFLOW, Vec,
                                     PetscScalar *); /* Objective */
  PetscErrorCode (*computegradient)(
      TCOPFLOW, Vec, Vec); /* Gradient of the objective function */
};
...
extern PetscErrorCode TCOPFLOWGetConstraintMultipliers(TCOPFLOW, PetscInt,
                                                       Vec *);
