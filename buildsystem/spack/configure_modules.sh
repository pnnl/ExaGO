...
# First install only the dependencies
spack install -j $1 --only dependencies && \
...
# Now try to install the exago package
spack install -j $1 --only package && \
spack module tcl refresh -y && \
# and populate the exago module
