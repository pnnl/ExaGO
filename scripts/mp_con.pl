...
#
      ...
      while ($nbus > 0) {
        ...
        for ($i=0; $i<$nbus; $i++) {
          $j = $top[$oldbus[$i]];
          while ($j >= 0) {
            if (!(($oldbus[$i]+1 == $bus1 && $nghbrs[$j]+1 == $bus2 && $nghbrtag[$j] == $tag)
              || ($oldbus[$i]+1 == $bus2 && $nghbrs[$j]+1 == $bus1 && $nghbrtag[$j] == $tag))) {
              $ob = $oldbus[$i]+1;
              $nb = $nghbrs[$j]+1;
              if ($found[$nghbrs[$j]] == 0) {
                $bcnt++;
      ...
      if ($bcnt == $buscnt) {
        ...
        print "Rejecting line contingency $bus1-$bus2 cnt: $bcnt nbus: $buscnt\n";
      }
    }
  }
}
      ...
      while ($nnbus > 0) {
        ...
        for ($i=0; $i<$nnbus; $i++) {
          ...
          while ($j >= 0) {
            ...
            $check1 = 0;
            if ($type eq "GL") {
              if (!(($oldbus[$i]+1 == $buses[1] && $nghbrs[$j]+1 == $buses[2] && $nghbrtag[$j] == $ltags[0])
                || ($oldbus[$i]+1 == $buses[2] && $nghbrs[$j]+1 == $buses[1] && $nghbrtag[$j] == $ltags[0]))) {
                $check1 = 1;
              }
            } elsif ($type eq "LL") {
              $ck1 = 0;
              if (!(($oldbus[$i]+1 == $buses[0] && $nghbrs[$j]+1 == $buses[1] && $nghbrtag[$j] == $ltags[0])
                || ($oldbus[$i]+1 == $buses[1] && $nghbrs[$j]+1 == $buses[0] && $nghbrtag[$j] == $ltags[0]))) {
                $ck1 = 1;
              }
              $ck2 = 0;
              if (!(($oldbus[$i]+1 == $buses[2] && $nghbrs[$j]+1 == $buses[3] && $nghbrtag[$j] == $ltags[1])
                || ($oldbus[$i]+1 == $buses[3] && $nghbrs[$j]+1 == $buses[2] && $nghbrtag[$j] == $ltags[1]))) {
                $ck2 = 1;
              }
              if ($ck1 && $ck2) {
                $check1 = 1;
    ...
    if ($check) {
      if ($type eq "GG") {
        if ($allGGC == 1 || $ngg < $maxGGC) {
          print CONTINGENCY "CONTINGENCY GG_$buses[0]\_$gtags[0]\_$buses[1]\_$gtags[1]_$count\n";
        ...
        if ($allGLC == 1 || $ngl < $maxGLC) {
          print CONTINGENCY "CONTINGENCY GL_$buses[0]\_$gtags[0]\_$buses[1]-$buses[2]\_$ltags[0]_$count\n";
          print CONTINGENCY "REMOVE UNIT $gtags[0] FROM BUS $buses[0]\n";
          print CONTINGENCY "OPEN BRANCH FROM BUS $buses[1] TO $buses[2] CIRCUT $ltags[0]\n";
          print CONTINGENCY "END\n";
          $count++;
          $ngl++;
        }
        ...
        if ($allLLC == 1 || $nll < $maxLLC) {
          print CONTINGENCY "CONTINGENCY LL_$buses[0]-$buses[1]\_$ltags[0]\_$buses[2]-$buses[3]\_$ltags[1]_$count\n";
          print CONTINGENCY "OPEN BRANCH FROM BUS $buses[0] TO $buses[1] CIRCUT $ltags[0]\n";
          print CONTINGENCY "OPEN BRANCH FROM BUS $buses[2] TO $buses[3] CIRCUT $ltags[1]\n";
          print CONTINGENCY "END\n";
          $count++;
          $nll++;
        }
      ...
      if ($type eq "GL") {
        print "Rejecting GL contingency $buses[1]-$buses[2] circuit: $ltags[0]\n";
      }
      if ($type eq "LL") {
        print "Rejecting LL contingency $buses[0]-$buses[1] circuit: $ltags[0] $buses[2]-$buses[3] circuit: $ltags[1]\n";
      }
    }
  }
}
