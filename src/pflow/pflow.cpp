...
PetscErrorCode PFLOWCreate(MPI_Comm mpicomm, PFLOW *pflowout) {
  ...
  PetscFunctionBegin;
  ierr = PetscCalloc1(1, &pflow);
  CHKERRQ(ierr);

  ierr = COMMCreate(mpicomm, &pflow->comm);
  CHKERRQ(ierr);

  ierr = PSCreate(mpicomm, &pflow->ps);
  CHKERRQ(ierr);

  /* Set the application with the PS object */
  ierr = PSSetApplication(pflow->ps, pflow, APP_ACPF);
  CHKERRQ(ierr);
...
PetscErrorCode PFLOWDestroy(PFLOW *pflow) {
  ...
  PetscFunctionBegin;
  ierr = COMMDestroy(&(*pflow)->comm);
  CHKERRQ(ierr);

  ierr = VecDestroy(&(*pflow)->X);
  CHKERRQ(ierr);
  ierr = MatDestroy(&(*pflow)->Jac);
  CHKERRQ(ierr);

  ierr = SNESDestroy(&(*pflow)->snes);
  CHKERRQ(ierr);

  ierr = PSDestroy(&(*pflow)->ps);
  CHKERRQ(ierr);

  ierr = PetscFree(*pflow);
  CHKERRQ(ierr);
...
PetscErrorCode PFLOWReadMatPowerData(PFLOW pflow, const char netfile[]) {
  ...
  ierr = PSReadMatPowerData(pflow->ps, netfile);
  CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
...
PetscErrorCode PFLOWCreateGlobalVector(PFLOW pflow, Vec *vec) {
  ...
  if (!pflow->setupcalled)
    SETERRQ(
        pflow->comm->type, 0,
        "PFLOWSetUp() must be called before calling PFLOWCreateGlobalVector");
  ierr = PSCreateGlobalVector(pflow->ps, vec);
  CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
...
PetscErrorCode PFLOWCreateMatrix(PFLOW pflow, Mat *mat) {
  ...
  PetscFunctionBegin;
  if (!pflow->setupcalled)
    SETERRQ(pflow->comm->type, 0,
            "PFLOWSetUp() must be called before calling PFLOWCreateMatrix");
  ierr = PSCreateMatrix(pflow->ps, mat);
  CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
  ...
  for (i = 0; i < ps->nbus; i++) {
    ...
    if (!ghostbus) {
      ...
      if (bus->ide != PV_BUS) {
        ...
        val[3] = 1.0; /* Partial derivative of voltage magnitude constraint */
      ierr = MatSetValues(J, 2, row, 2, col, val, ADD_VALUES);
      CHKERRQ(ierr);
    }
...
PetscErrorCode PFLOWFunction(SNES snes, Vec X, Vec F, void *ctx) {
  ...
  ierr = DMGetLocalVector(ps->networkdm, &localF);
  CHKERRQ(ierr);
  ierr = DMGlobalToLocalBegin(ps->networkdm, F, INSERT_VALUES, localF);
  CHKERRQ(ierr);
  ierr = DMGlobalToLocalEnd(ps->networkdm, F, INSERT_VALUES, localF);
  CHKERRQ(ierr);
  ...
  ierr = DMLocalToGlobalBegin(ps->networkdm, localF, ADD_VALUES, F);
  CHKERRQ(ierr);
  ierr = DMLocalToGlobalEnd(ps->networkdm, localF, ADD_VALUES, F);
  CHKERRQ(ierr);

  ierr = DMRestoreLocalVector(ps->networkdm, &localX);
  CHKERRQ(ierr);
  ierr = DMRestoreLocalVector(ps->networkdm, &localF);
  CHKERRQ(ierr);
  ierr = PetscLogFlops(flps);
  CHKERRQ(ierr);
  ...
  ierr =
  DMGlobalToLocalBegin(ps->networkdm,F,INSERT_VALUES,localF);CHKERRQ(ierr); ierr
  = DMGlobalToLocalEnd(ps->networkdm,F,INSERT_VALUES,localF);CHKERRQ(ierr); ierr
  = VecGetArray(localF,&farr);CHKERRQ(ierr);
  ...
  for(i=0; i < ps->nbus; i++) {
    ...
    if(!ghostbus) {
      ierr = PetscPrintf(PETSC_COMM_SELF,"Rank %d Bus %d ftheta %5.4f fVm
...
PetscErrorCode PFLOWSetInitialGuess(PFLOW pflow, Vec X) {
  ...
  for (i = 0; i < nbus; i++) {
    ...
    //    ierr = PetscPrintf(PETSC_COMM_SELF,"Rank %d Bus %d Type %d Vm %4.3f Va
...
PetscErrorCode PFLOWSolve(PFLOW pflow) {
  ...
  /* Check Topology */
  ierr = PSCheckTopology(pflow->ps);
  CHKERRQ(ierr);

  /* Set up initial guess */
  ierr = PFLOWSetInitialGuess(pflow, pflow->X);
  CHKERRQ(ierr);

  /* Call the nonlinear solver */
  ierr = SNESSolve(pflow->snes, NULL, pflow->X);
  CHKERRQ(ierr);
...
PetscErrorCode PFLOWPostSolve(PFLOW pflow) {
  ...
  /* Create global vector PQgen of size 2*nbus X 1 for storing Pgen and Qgen at
   * each bus */
  ierr = PFLOWCreateGlobalVector(pflow, &PQgen);
  CHKERRQ(ierr);
  ierr = VecSet(PQgen, 0.0);
  CHKERRQ(ierr);
  ierr = DMGetLocalVector(ps->networkdm, &localPQgen);
  CHKERRQ(ierr);
  ierr = DMGlobalToLocalBegin(ps->networkdm, PQgen, INSERT_VALUES, localPQgen);
  CHKERRQ(ierr);
  ierr = DMGlobalToLocalEnd(ps->networkdm, PQgen, INSERT_VALUES, localPQgen);
  CHKERRQ(ierr);
  ierr = VecGetArray(localPQgen, &pqgenarr);
  CHKERRQ(ierr);

  ierr = DMGetLocalVector(ps->networkdm, &localX);
  CHKERRQ(ierr);
  ierr = DMGlobalToLocalBegin(ps->networkdm, pflow->X, INSERT_VALUES, localX);
  CHKERRQ(ierr);
  ierr = DMGlobalToLocalEnd(ps->networkdm, pflow->X, INSERT_VALUES, localX);
  CHKERRQ(ierr);
  ierr = VecGetArrayRead(localX, &xarr);
  CHKERRQ(ierr);
  ...
  for (i = 0; i < ps->nbus; i++) {
    ...
    flps += 2;
    if (!ghostbus) {
      //      ierr = PetscPrintf(PETSC_COMM_SELF,"Rank %d Bus %d Type %d Vm
      //      %4.3f Va
  ...
  ierr = VecRestoreArray(localPQgen, &pqgenarr);
  CHKERRQ(ierr);
  ierr = DMLocalToGlobalBegin(ps->networkdm, localPQgen, ADD_VALUES, PQgen);
  CHKERRQ(ierr);
  ierr = DMLocalToGlobalEnd(ps->networkdm, localPQgen, ADD_VALUES, PQgen);
  CHKERRQ(ierr);
  ...
  for (i = 0; i < ps->nbus; i++) {
    ...
    if (!pflow->split_gen_power_within_limits) {
      ...
      if (bus->ide == REF_BUS || bus->ide == PV_BUS) {
        temptotal = 0;
        pgtotalnonlt =
            bus->Pgtot;  // Net Pg for generators not on VAR limits on this bus.
                         // This will be needed later if some of the buses are
                         // VAR limited during splitting.
        qgtotalonlt = 0; // Net Qg for generators on Var limit on the bus
        MVAbasenonlt = bus->MVAbasetot; // Net MVAbase for generators not on VAR
                                        // limits on this bus.

        if (bus->ngen ==
            1) { // If there is only one generator, do not split, this avoids
                 // that generator being VAR limited at the splitting stage. VAR
                 // limits will still be an issue if the net VAR at the bus is
                 // beyond its net VAR limit.
          PSGEN gen;
          ierr = PSBUSGetGen(bus, 0, &gen);
          CHKERRQ(ierr);
          if (bus->ide == REF_BUS) {
            gen->pg = pqgenarr[loc];
          }
          ...
          for (k = 0; k < bus->ngen; k++) {
            ...
            if (gen->status) {
              if (bus->ide == REF_BUS) {
                if (bus->Pgtot != 0) {
                  gen->pg =
                      pqgenarr[loc] * PetscAbsScalar(gen->pg) / bus->Pgtot;
                } else {
                  gen->pg = pqgenarr[loc] * gen->mbase / bus->MVAbasetot;
                }
                flps += 2;
              }
              if (PetscAbsScalar(gen->qt - gen->qb) < PETSC_SMALL)
                gen->qg = gen->qt; /* Fixed VAR generator */
              else {
                if (bus->Pgtot != 0) {
                  gen->qg = pqgenarr[loc + 1] * PetscAbsScalar(gen->pg) /
                            bus->Pgtot; /* Proportional distribution */
                } else {
                  gen->qg = pqgenarr[loc + 1] * gen->mbase / bus->MVAbasetot;
                }
                flps += 2;
                if (gen->qg >
                    gen->qt) { // In PSSE while splitting the VARS on the
                               // generators, VAR limits are respected.
                  gen->qg = gen->qt;
                  pgtotalnonlt -= PetscAbsScalar(
                      gen->pg); // Substract this PG from net PG not on limit
                  qgtotalonlt += gen->qg;     // Add this Qg to net Qg on limit
                  MVAbasenonlt -= gen->mbase; // Substract this MVAbase from net
                                              // MVAbase not on limit
                  flps += 3;
            ...
            temptotal +=
                gen->qg; // Calculate the total Qg on the bus, since some of the
                         // generators might be VAR limited, need to check if
                         // the total Qg is still the same as the total QG
                         // before splitting.
          }
          flps += bus->ngen; // for l852
        }
        diffqg =
            pqgenarr[loc + 1] - temptotal; // Calculate the difference between
                                           // net QG before and after splitting.
        if (PetscAbsScalar(diffqg) >
            0.0001) { // If there is a difference between the net Qg before and
                      // after splitting, distribute the difference on
                      // generators  which are not on VAR limits proportional to
                      // their PG output (or MVAbase if net PG output of
                      // generators not on VAR limits is zero.)
          for (k = 0; k < bus->ngen; k++) {
            PSGEN gen;
            ierr = PSBUSGetGen(bus, k, &gen);
            CHKERRQ(ierr);
            if (PetscAbsScalar(gen->qg - gen->qt) > PETSC_SMALL &&
                PetscAbsScalar(gen->qg - gen->qb) > PETSC_SMALL) {
              if (PetscAbsScalar(pgtotalnonlt - 0) > PETSC_SMALL) {
                gen->qg = (pqgenarr[loc + 1] - qgtotalonlt) *
                          PetscAbsScalar(gen->pg) / pgtotalnonlt;
              } else {
                gen->qg = (pqgenarr[loc + 1] - qgtotalonlt) * gen->mbase /
                          MVAbasenonlt;
              }
              flps += 3;
            }
...
PetscErrorCode PFLOWSetUp(PFLOW pflow) {
  ...
  ierr = PSCreateGlobalVector(pflow->ps, &pflow->X);
  CHKERRQ(ierr);
  ierr = PSCreateMatrix(pflow->ps, &pflow->Jac);
  CHKERRQ(ierr);

  /* Associate the DM object in PS with the SNES solver */
  ierr = SNESSetDM(pflow->snes, pflow->ps->networkdm);
  CHKERRQ(ierr);

  /* Set the prefix for this SNES.. All SNES runtime options will need to have
   * the prefix "-pflow_" */
  ierr = SNESSetOptionsPrefix(pflow->snes, "pflow_");
  CHKERRQ(ierr);

  /* Set the function and Jacobian routines */
  ierr = SNESSetFunction(pflow->snes, NULL, PFLOWFunction, (void *)pflow);
  CHKERRQ(ierr);
...
*/
PetscErrorCode PFLOWSetLineStatus(PFLOW pflow, PetscInt fbus, PetscInt tbus,
                                  const char *id, PetscInt status) {
  PetscErrorCode ierr;

  PetscFunctionBegin;
  ierr = PSSetLineStatus(pflow->ps, fbus, tbus, id, status);
  CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
...
*/
PetscErrorCode PFLOWSetGenStatus(PFLOW pflow, PetscInt gbus, const char *gid,
                                 PetscInt status) {
  PetscErrorCode ierr;

  PetscFunctionBegin;
  ierr = PSSetGenStatus(pflow->ps, gbus, gid, status);
  CHKERRQ(ierr);
  PetscFunctionReturn(0);
}
...
*/
PetscErrorCode PFLOWGetBusVoltage(PFLOW pflow, PetscInt busnum, PetscScalar *Vm,
                                  PetscScalar *Va, PetscBool *found) {
  PetscInt intbusnum;
  PS ps = pflow->ps;
  PSBUS bus;
...
*/
PetscErrorCode PFLOWSetBusVoltage(PFLOW pflow, PetscInt busnum, PetscScalar Vm,
                                  PetscScalar Va) {
  PetscErrorCode ierr;
  PetscInt intbusnum;
  PS ps = pflow->ps;
...
/*
  PFLOWGetGenDispatch - Gets the real and reactive power output for generator at
a given bus
...
Notes: PFLOWSetUp() must be called before calling PFLOWSetBusVoltage()
*/
PetscErrorCode PFLOWGetGenDispatch(PFLOW pflow, PetscInt busnum,
                                   PetscInt gennum, PetscScalar *Pg,
                                   PetscScalar *Qg, PetscBool *found) {
  PetscErrorCode ierr;
  PetscInt intbusnum;
  PS ps = pflow->ps;
  PSBUS bus;
  ...
  if (intbusnum != -1) {
    bus = &ps->bus[intbusnum];
    if (!bus->ngen) {
      SETERRQ1(PETSC_COMM_SELF, PETSC_ERR_SUP, "No generators on bus %d",
               busnum);
    }
    if (bus->ide == ISOLATED_BUS) {
      SETERRQ1(PETSC_COMM_SELF, PETSC_ERR_SUP, "Bus %d is isolated", busnum);
    }
    ierr = PSBUSGetGen(bus, gennum, &gen);
    CHKERRQ(ierr);
...
/*
  PFLOWSetGenDispatch - Sets the real and reactive power output for generator at
a given bus
...
*/
PetscErrorCode PFLOWSetGenDispatch(PFLOW pflow, PetscInt busnum,
                                   PetscInt gennum, PetscScalar Pg,
                                   PetscScalar Qg) {
  PetscErrorCode ierr;
  PetscInt intbusnum;
...
*/
PetscErrorCode PFLOWSetLoadPower(PFLOW pflow, PetscInt busnum, PetscScalar Pd,
                                 PetscScalar Qd) {
  PetscErrorCode ierr;
  PetscInt intbusnum;
  PS ps = pflow->ps;
...
*/
PetscErrorCode PFLOWGetLoadPower(PFLOW pflow, PetscInt busnum, PetscScalar *Pd,
                                 PetscScalar *Qd, PetscBool *found) {
  PetscErrorCode ierr;
  PetscInt intbusnum;
  PS ps = pflow->ps;
  PSBUS bus;
...
*/
PetscErrorCode PFLOWAddBusShunt(PFLOW pflow, PetscInt busnum, PetscScalar Gs,
                                PetscScalar Bs) {
  PetscErrorCode ierr;
  PetscInt intbusnum;
  PS ps = pflow->ps;
