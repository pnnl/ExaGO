...
PetscErrorCode SCOPFLOWSaveSolutionAll(SCOPFLOW scopflow,OutputFormat format,const char outdir[])
{
  ...
  for(c=0; c < scopflow->nc; c++) {
    if(scopflow->ismultiperiod) {
      ...
      ierr = PetscSNPrintf(tcopflowdirname,64,"cont_%d",scopflow->cstart+c);CHKERRQ(ierr);
      ierr = PetscStrncpy(tcopflowdir,outdir,256);CHKERRQ(ierr);
      ierr = PetscStrlcat(tcopflowdir,"/",256);CHKERRQ(ierr);
      ierr = PetscStrlcat(tcopflowdir,tcopflowdirname,256);CHKERRQ(ierr);
	...
	ierr = PetscMkdir(tcopflowdir);CHKERRQ(ierr);

	ierr = PetscSNPrintf(filename,64,"t%d.m",t);CHKERRQ(ierr);
	ierr = PetscStrncpy(outfile,tcopflowdir,256);CHKERRQ(ierr);
	...
	ierr = VecResetArray(opflow->Lambda);
      ...
      opflow = scopflow->opflows[c];
      if(!opflow->solutiontops) {
	ierr = SCOPFLOWGetSolution(scopflow,scopflow->cstart+c,&opflow->X);CHKERRQ(ierr);
	ierr = SCOPFLOWGetConstraintMultipliers(scopflow,scopflow->cstart+c,&opflow->Lambda);CHKERRQ(ierr);
	ierr = OPFLOWSolutionToPS(opflow);CHKERRQ(ierr);
      }
      ierr = PetscSNPrintf(filename,64,"cont_%d",scopflow->cstart+c);CHKERRQ(ierr);
