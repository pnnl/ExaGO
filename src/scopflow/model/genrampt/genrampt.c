...
PetscErrorCode SCOPFLOWSetVariableBounds_GENRAMPT(SCOPFLOW scopflow,Vec Xl,Vec Xu)
{
  PetscErrorCode ierr;
  TCOPFLOW       tcopflow;
  ...
  for(i=0; i < scopflow->Nc; i++) {
    ...
    ierr = VecPlaceArray(tcopflow->Xl,xli);CHKERRQ(ierr);
    ierr = VecPlaceArray(tcopflow->Xu,xui);CHKERRQ(ierr);

    /* Set bounds */
    ierr = (*tcopflow->modelops.setvariablebounds)(tcopflow,tcopflow->Xl,tcopflow->Xu);CHKERRQ(ierr);

    ierr = VecResetArray(tcopflow->Xl);CHKERRQ(ierr);
    ierr = VecResetArray(tcopflow->Xu);CHKERRQ(ierr);
  }
...
PetscErrorCode SCOPFLOWSetConstraintBounds_GENRAMPT(SCOPFLOW scopflow,Vec Gl,Vec Gu)
{
  PetscInt       i,j,k,ctr;
  PetscErrorCode ierr;
  ...
  for(i=0; i < scopflow->Nc; i++) {
    ...
    ierr = VecPlaceArray(tcopflow->Gl,gli);CHKERRQ(ierr);
    ierr = VecPlaceArray(tcopflow->Gu,gui);CHKERRQ(ierr);
	...
	for(k=0; k < bus->ngen; k++) {
	  ...
	  /* Ramp constraints */
	  gli[tcopflow->Ncon + ctr] = -gen->ramp_rate_30min;
	  gui[tcopflow->Ncon + ctr] =  gen->ramp_rate_30min;
	  ctr++;
	}
      }
    }
...
PetscErrorCode SCOPFLOWSetVariableandConstraintBounds_GENRAMPT(SCOPFLOW scopflow,Vec Xl,Vec Xu, Vec Gl, Vec Gu)
{
  PetscErrorCode ierr;

  PetscFunctionBegin;

  ierr = SCOPFLOWSetVariableBounds_GENRAMPT(scopflow,Xl,Xu);CHKERRQ(ierr);
  ierr = SCOPFLOWSetConstraintBounds_GENRAMPT(scopflow,Gl,Gu);CHKERRQ(ierr);
...
PetscErrorCode SCOPFLOWSetInitialGuess_GENRAMPT(SCOPFLOW scopflow,Vec X)
{
  ...
  for(i=0; i < scopflow->Nc; i++) {
    ...
    ierr = VecPlaceArray(tcopflow->X,xi);CHKERRQ(ierr);
    ierr = VecPlaceArray(tcopflow->Xl,xli);CHKERRQ(ierr);
    ierr = VecPlaceArray(tcopflow->Xu,xui);CHKERRQ(ierr);
    ...
    ierr = VecResetArray(tcopflow->X);CHKERRQ(ierr);
    ierr = VecResetArray(tcopflow->Xl);CHKERRQ(ierr);
    ierr = VecResetArray(tcopflow->Xu);CHKERRQ(ierr);
  }
...
PetscErrorCode SCOPFLOWComputeJacobian_GENRAMPT(SCOPFLOW scopflow,Vec X,Mat J)
{
  PetscErrorCode ierr;
  TCOPFLOW       tcopflow,tcopflow0;
  OPFLOW         opflow,opflow0;
  ...
  for(i=0; i < scopflow->Nc; i++) {
    ...
    ierr = (*tcopflow->modelops.computejacobian)(tcopflow,tcopflow->X,tcopflow->Jac);CHKERRQ(ierr);

    ierr = MatGetSize(tcopflow->Jac,&nrow,&ncol);CHKERRQ(ierr);
    /* Copy over locations and values to Jac */
    for(j=0; j < nrow; j++) {
      ierr = MatGetRow(tcopflow->Jac,j,&nvals,&cols,&vals);CHKERRQ(ierr);
      for(k=0; k < nvals; k++) {
	row = roffset + j;
	col = coffset + cols[k];
	val = vals[k];
	ierr = MatSetValues(J,1,&row,1,&col,&val,INSERT_VALUES);
      ...
      ierr = MatRestoreRow(tcopflow->Jac,j,&nvals,&cols,&vals);CHKERRQ(ierr);
    }
...
PetscErrorCode SCOPFLOWComputeConstraints_GENRAMPT(SCOPFLOW scopflow,Vec X,Vec G)
{
  PetscErrorCode ierr;
  ...
  for(i=0; i < scopflow->Nc; i++) {
    ...
    ierr = VecPlaceArray(tcopflow->X,xi);CHKERRQ(ierr);
    ierr = VecPlaceArray(tcopflow->G,gi);CHKERRQ(ierr);

    ierr = (*tcopflow->modelops.computeconstraints)(tcopflow,tcopflow->X,tcopflow->G);CHKERRQ(ierr);

    ierr = VecResetArray(tcopflow->X);CHKERRQ(ierr);
    ierr = VecResetArray(tcopflow->G);CHKERRQ(ierr);
...
PetscErrorCode SCOPFLOWComputeObjective_GENRAMPT(SCOPFLOW scopflow,Vec X,PetscScalar *obj)
{
  PetscErrorCode ierr;
  TCOPFLOW       tcopflow;
  PetscInt       i;
  ...
  *obj = 0.0;
  ...
  for(i=0; i < scopflow->Nc; i++) {
    ...
    tcopflow = scopflow->tcopflows[i];
    ierr = VecPlaceArray(tcopflow->X,xi);CHKERRQ(ierr);
    ierr = (*tcopflow->modelops.computeobjective)(tcopflow,tcopflow->X,&tcopflowobj);CHKERRQ(ierr);
    *obj += tcopflowobj;
    ierr = VecResetArray(tcopflow->X);CHKERRQ(ierr);
  }
  ierr = VecRestoreArray(X,&x);CHKERRQ(ierr);
...
PetscErrorCode SCOPFLOWComputeGradient_GENRAMPT(SCOPFLOW scopflow,Vec X,Vec Grad)
{
  PetscErrorCode ierr;
  TCOPFLOW         tcopflow;
  PetscInt       i;
  ...
  for(i=0; i < scopflow->Nc; i++) {
    ...
    ierr = VecPlaceArray(tcopflow->X,xi);CHKERRQ(ierr);
    ierr = VecPlaceArray(tcopflow->gradobj,gradi);CHKERRQ(ierr);
    ierr = VecSet(tcopflow->gradobj,0.0);CHKERRQ(ierr);

    ierr = (*tcopflow->modelops.computegradient)(tcopflow,tcopflow->X,tcopflow->gradobj);CHKERRQ(ierr);
    
    ierr = VecResetArray(tcopflow->X);CHKERRQ(ierr);
    ierr = VecResetArray(tcopflow->gradobj);CHKERRQ(ierr);
  }
...
PetscErrorCode SCOPFLOWComputeObjandGradient_GENRAMPT(SCOPFLOW scopflow,Vec X,PetscScalar *obj,Vec Grad)
{
  PetscErrorCode ierr;
  PetscFunctionBegin;

  ierr = SCOPFLOWComputeObjective_GENRAMPT(scopflow,X,obj);CHKERRQ(ierr);
  ierr = SCOPFLOWComputeGradient_GENRAMPT(scopflow,X,Grad);CHKERRQ(ierr);
...
PetscErrorCode SCOPFLOWModelSetNumVariablesandConstraints_GENRAMPT(SCOPFLOW scopflow,PetscInt *nxi,PetscInt *ngi,PetscInt *nconineqcoup)
{
  ...
  for(c=0; c < scopflow->nc; c++) {
    ...
    nxi[c] = tcopflow->Nx;
    if(scopflow->iscoupling) nconineqcoup[c] = (c == 0)?0:ngenON;
    ngi[c] = tcopflow->Ncon + nconineqcoup[c];
  }
  PetscFunctionReturn(0);
}

PetscErrorCode SCOPFLOWComputeHessian_GENRAMPT(SCOPFLOW scopflow,Vec X,Vec Lambda,Mat H)
{
  PetscErrorCode    ierr;
  PetscInt          nrow;
  TCOPFLOW          tcopflow;
  ...
  for(i=0; i < scopflow->Nc; i++) {
    ...
    ierr = VecPlaceArray(tcopflow->X,xi);CHKERRQ(ierr);
    ierr = VecPlaceArray(tcopflow->Lambda,lambdai);CHKERRQ(ierr);

    ierr = (*tcopflow->modelops.computehessian)(tcopflow,tcopflow->X,tcopflow->Lambda,tcopflow->Hes);CHKERRQ(ierr);

    ierr = VecResetArray(tcopflow->X);CHKERRQ(ierr);
    ierr = VecResetArray(tcopflow->Lambda);CHKERRQ(ierr);

    /* Copy over values */
    ierr = MatGetSize(tcopflow->Hes,&nrow,&nrow);CHKERRQ(ierr);
    for(j=0; j < nrow; j++) {
      ierr = MatGetRow(tcopflow->Hes,j,&nvals,&cols,&vals);CHKERRQ(ierr);
      row = roffset + j;
      for(k=0; k < nvals; k++) {
	col = roffset + cols[k];
	val = vals[k];
	ierr = MatSetValues(H,1,&row,1,&col,&val,INSERT_VALUES);CHKERRQ(ierr);
      ...
      ierr = MatRestoreRow(tcopflow->Hes,j,&nvals,&cols,&vals);CHKERRQ(ierr);
    }
  }
...
PetscErrorCode SCOPFLOWModelCreate_GENRAMPT(SCOPFLOW scopflow)
{
  ...
  ierr = PetscCalloc1(1,&genrampt);CHKERRQ(ierr);

  scopflow->model = genrampt;

  /* Inherit Ops */
  scopflow->modelops.destroy = SCOPFLOWModelDestroy_GENRAMPT;
  scopflow->modelops.setnumvariablesandconstraints = SCOPFLOWModelSetNumVariablesandConstraints_GENRAMPT;
  ...
  scopflow->modelops.setvariableandconstraintbounds = SCOPFLOWSetVariableandConstraintBounds_GENRAMPT;
  ...
  scopflow->modelops.computeobjandgradient = SCOPFLOWComputeObjandGradient_GENRAMPT;
  scopflow->modelops.computeobjective = SCOPFLOWComputeObjective_GENRAMPT;
