...
PetscErrorCode OPFLOWModelDestroy_PBCAR(OPFLOW opflow) {
  PetscErrorCode ierr;
...
PetscErrorCode OPFLOWSetVariableBounds_PBCAR(OPFLOW opflow, Vec Xl, Vec Xu) {
  ...
  ierr = DMGetLocalVector(ps->networkdm, &localXl);
  CHKERRQ(ierr);
  ierr = DMGetLocalVector(ps->networkdm, &localXu);
  CHKERRQ(ierr);
  ierr = VecSet(Xl, 0.0);
  CHKERRQ(ierr);
  ierr = VecSet(Xu, 0.0);
  CHKERRQ(ierr);
  ierr = DMGlobalToLocalBegin(ps->networkdm, Xl, INSERT_VALUES, localXl);
  CHKERRQ(ierr);
  ierr = DMGlobalToLocalEnd(ps->networkdm, Xl, INSERT_VALUES, localXl);
  CHKERRQ(ierr);
  ierr = DMGlobalToLocalBegin(ps->networkdm, Xu, INSERT_VALUES, localXu);
  CHKERRQ(ierr);
  ierr = DMGlobalToLocalEnd(ps->networkdm, Xu, INSERT_VALUES, localXu);
  CHKERRQ(ierr);

  /* Get array pointers */
  ierr = VecGetArray(localXl, &xl);
  CHKERRQ(ierr);
  ierr = VecGetArray(localXu, &xu);
  CHKERRQ(ierr);
  ...
  ierr = VecRestoreArray(localXl, &xl);
  CHKERRQ(ierr);
  ierr = VecRestoreArray(localXu, &xu);
  CHKERRQ(ierr);

  ierr = DMLocalToGlobalBegin(ps->networkdm, localXl, ADD_VALUES, Xl);
  CHKERRQ(ierr);
  ierr = DMLocalToGlobalEnd(ps->networkdm, localXl, ADD_VALUES, Xl);
  CHKERRQ(ierr);
  ierr = DMLocalToGlobalBegin(ps->networkdm, localXu, ADD_VALUES, Xu);
  CHKERRQ(ierr);
  ierr = DMLocalToGlobalEnd(ps->networkdm, localXu, ADD_VALUES, Xu);
  CHKERRQ(ierr);

  ierr = DMRestoreLocalVector(ps->networkdm, &localXl);
  CHKERRQ(ierr);
  ierr = DMRestoreLocalVector(ps->networkdm, &localXu);
  CHKERRQ(ierr);
...
PetscErrorCode OPFLOWSetVariableandConstraintBounds_PBCAR(OPFLOW opflow, Vec Xl,
                                                          Vec Xu, Vec Gl,
                                                          Vec Gu) {
  PetscErrorCode ierr;
  PetscFunctionBegin;

  ierr = OPFLOWSetVariableBounds_PBCAR(opflow, Xl, Xu);
  CHKERRQ(ierr);
  ierr = OPFLOWSetConstraintBounds_PBCAR(opflow, Gl, Gu);
  CHKERRQ(ierr);
...
PetscErrorCode OPFLOWSetInitialGuess_PBCAR(OPFLOW opflow, Vec X, Vec Lambda) {
  ...
  ierr = DMGlobalToLocalBegin(ps->networkdm, X, INSERT_VALUES, localX);
  CHKERRQ(ierr);
  ierr = DMGlobalToLocalEnd(ps->networkdm, X, INSERT_VALUES, localX);
  CHKERRQ(ierr);
  ierr =
      DMGlobalToLocalBegin(ps->networkdm, opflow->Xl, INSERT_VALUES, localXl);
  CHKERRQ(ierr);
  ierr = DMGlobalToLocalEnd(ps->networkdm, opflow->Xl, INSERT_VALUES, localXl);
  CHKERRQ(ierr);
  ierr =
      DMGlobalToLocalBegin(ps->networkdm, opflow->Xu, INSERT_VALUES, localXu);
  CHKERRQ(ierr);
  ierr = DMGlobalToLocalEnd(ps->networkdm, opflow->Xu, INSERT_VALUES, localXu);
  CHKERRQ(ierr);

  /* Get array pointers */
  ierr = VecGetArray(localX, &x);
  CHKERRQ(ierr);
  ierr = VecGetArrayRead(localXl, &xl);
  CHKERRQ(ierr);
  ierr = VecGetArrayRead(localXu, &xu);
  CHKERRQ(ierr);
  ...
  CHKERRQ(ierr);
  ierr = VecRestoreArrayRead(localXl, &xl);
  CHKERRQ(ierr);
  ierr = VecRestoreArrayRead(localXu, &xu);
  CHKERRQ(ierr);

  ierr = DMLocalToGlobalBegin(ps->networkdm, localX, ADD_VALUES, X);
  CHKERRQ(ierr);
  ierr = DMLocalToGlobalEnd(ps->networkdm, localX, ADD_VALUES, X);
  CHKERRQ(ierr);
...
PetscErrorCode OPFLOWComputeEqualityConstraints_PBCAR(OPFLOW opflow, Vec X,
                                                      Vec Ge) {
  PetscErrorCode ierr;
  ...
  ierr = VecSet(Ge, 0.0);
  CHKERRQ(ierr);
  ierr = VecSet(opflow->Gelocal, 0.0);
  CHKERRQ(ierr);
  ...
  ierr = VecGetArrayRead(localX, &x);
  CHKERRQ(ierr);
  ierr = VecGetArray(opflow->Gelocal, &ge);
  CHKERRQ(ierr);
  ...
  ierr = VecRestoreArrayRead(localX, &x);
  CHKERRQ(ierr);
  ierr = DMRestoreLocalVector(ps->networkdm, &localX);
  CHKERRQ(ierr);

  ierr = VecRestoreArray(opflow->Gelocal, &ge);
  CHKERRQ(ierr);

  ierr = VecScatterBegin(opflow->scattereqcon, opflow->Gelocal, Ge, ADD_VALUES,
                         SCATTER_REVERSE);
  CHKERRQ(ierr);
  ierr = VecScatterEnd(opflow->scattereqcon, opflow->Gelocal, Ge, ADD_VALUES,
                       SCATTER_REVERSE);
  CHKERRQ(ierr);
...
PetscErrorCode OPFLOWComputeEqualityConstraintJacobian_PBCAR(OPFLOW opflow,
                                                             Vec X, Mat Je) {
  PetscErrorCode ierr;
  ...
  for (i = 0; i < ps->nbus; i++) {
    ...
    for (k = 0; k < nconnlines; k++) {
      ...
      flps += 36; // Same no. of operations in both the scopes.
    }
...
PetscErrorCode OPFLOWComputeInequalityConstraints_PBCAR(OPFLOW opflow, Vec X,
                                                        Vec Gi) {
  PetscErrorCode ierr;
  PetscInt i;
  PetscInt gloc = 0;
...
PetscErrorCode OPFLOWComputeInequalityConstraintJacobian_PBCAR(OPFLOW opflow,
                                                               Vec X, Mat Ji) {
  PetscErrorCode ierr;
  PetscInt i;
  ...
  for (i = 0; i < ps->nline; i++) {
    ...
    ierr = PSBUSGetVariableLocation(bust, &xloct);
    CHKERRQ(ierr);
    ierr = PSBUSGetVariableGlobalLocation(busf, &xlocfglob);
    CHKERRQ(ierr);
    ierr = PSBUSGetVariableGlobalLocation(bust, &xloctglob);
    CHKERRQ(ierr);
...
PetscErrorCode OPFLOWComputeObjective_PBCAR(OPFLOW opflow, Vec X,
                                            PetscScalar *obj) {
  PetscErrorCode ierr;
  const PetscScalar *x;
  PS ps = opflow->ps;
  ...
  for (i = 0; i < ps->nbus; i++) {
    ...
    if (opflow->include_loadloss_variables) {
      ...
      for (k = 0; k < bus->nload; k++) {
        ...
        Pdloss = x[loc];
        Qdloss = x[loc + 1];
        localobj += opflow->loadloss_penalty * ps->MVAbase * ps->MVAbase *
    ...
    if (opflow->include_powerimbalance_variables) {
      ...
      Pimb = x[loc];
      Qimb = x[loc + 1];
      localobj += opflow->powerimbalance_penalty * ps->MVAbase * ps->MVAbase *
                  (Pimb * Pimb + Qimb * Qimb);
      flps += 7;
    }
  }
  ...
  ierr =
      MPI_Allreduce(&localobj, obj, 1, MPIU_REAL, MPI_SUM, opflow->comm->type);
  CHKERRQ(ierr);
  ierr = PetscLogFlops(flps);
  CHKERRQ(ierr);
...
PetscErrorCode OPFLOWComputeGradient_PBCAR(OPFLOW opflow, Vec X, Vec grad) {
  ...
  ierr = VecSet(grad, 0.0);
  CHKERRQ(ierr);

  ierr = DMGetLocalVector(ps->networkdm, &localX);
  CHKERRQ(ierr);
  ierr = DMGetLocalVector(ps->networkdm, &localgrad);
  CHKERRQ(ierr);
  ...
  ierr = DMGlobalToLocalEnd(ps->networkdm, X, INSERT_VALUES, localX);
  CHKERRQ(ierr);
  ierr = DMGlobalToLocalBegin(ps->networkdm, grad, INSERT_VALUES, localgrad);
  CHKERRQ(ierr);
  ierr = DMGlobalToLocalEnd(ps->networkdm, grad, INSERT_VALUES, localgrad);
  CHKERRQ(ierr);

  ierr = VecGetArrayRead(localX, &x);
  CHKERRQ(ierr);
  ierr = VecGetArray(localgrad, &df);
  CHKERRQ(ierr);
  ...
  ierr = VecRestoreArrayRead(localX, &x);
  CHKERRQ(ierr);
  ierr = VecRestoreArray(localgrad, &df);
  CHKERRQ(ierr);

  ierr = DMLocalToGlobalBegin(ps->networkdm, localgrad, INSERT_VALUES, grad);
  CHKERRQ(ierr);
  ierr = DMLocalToGlobalEnd(ps->networkdm, localgrad, INSERT_VALUES, grad);
  CHKERRQ(ierr);

  ierr = DMRestoreLocalVector(ps->networkdm, &localX);
  CHKERRQ(ierr);
  ierr = DMRestoreLocalVector(ps->networkdm, &localgrad);
  CHKERRQ(ierr);
  ierr = PetscLogFlops(flps);
  CHKERRQ(ierr);
...
PetscErrorCode OPFLOWComputeObjandGradient_PBCAR(OPFLOW opflow, Vec X,
  ...
  CHKERRQ(ierr);
  ierr = OPFLOWComputeObjective_PBCAR(opflow, X, obj);
  CHKERRQ(ierr);
  ...
  CHKERRQ(ierr);
  ierr = OPFLOWComputeGradient_PBCAR(opflow, X, Grad);
  CHKERRQ(ierr);
...
PetscErrorCode OPFLOWModelSetNumVariables_PBCAR(OPFLOW opflow,
                                                PetscInt *busnvar,
                                                PetscInt *branchnvar,
                                                PetscInt *nx) {
  ...
  *nx = 0;
  ...
  for (i = 0; i < ps->nbus; i++) {
    ...
    for (j = 0; j < ngen; j++) {
      ...
      CHKERRQ(ierr);
      if (gen->status)
        busnvar[i] += 2; /* (2 variables Pg, Qg for each in-service gen) */
    }
    if (opflow->include_loadloss_variables) {
...
PetscErrorCode OPFLOWModelSetNumConstraints_PBCAR(OPFLOW opflow,
...
*/
PetscErrorCode OPFLOWComputeEqualityConstraintsHessian_PBCAR(OPFLOW opflow,
                                                             Vec X, Vec Lambda,
                                                             Mat H) {
  PetscErrorCode ierr;
  PS ps = opflow->ps;
  ...
  ierr = VecScatterBegin(opflow->scattereqcon, Lambda, opflow->Lambdaelocal,
                         INSERT_VALUES, SCATTER_FORWARD);
  CHKERRQ(ierr);
  ierr = VecScatterEnd(opflow->scattereqcon, Lambda, opflow->Lambdaelocal,
                       INSERT_VALUES, SCATTER_FORWARD);
  CHKERRQ(ierr);

  ierr = VecGetArrayRead(localX, &x);
  CHKERRQ(ierr);
  ierr = VecGetArrayRead(opflow->Lambdaelocal, &lambda);
  CHKERRQ(ierr);
  ...
  for (i = 0; i < ps->nbus; i++) {
    ...
    for (k = 0; k < nconnlines; k++) {
      ...
      if (bus == busf) {
        ...
        val[0] =
            lambda[gloc] * dPf_dVrf_dVrf + lambda[gloc + 1] * dQf_dVrf_dVrf;
        val[1] =
            lambda[gloc] * dPf_dVif_dVrf + lambda[gloc + 1] * dQf_dVif_dVrf;
        val[2] =
            lambda[gloc] * dPf_dVrt_dVrf + lambda[gloc + 1] * dQf_dVrt_dVrf;
        val[3] =
            lambda[gloc] * dPf_dVit_dVrf + lambda[gloc + 1] * dQf_dVit_dVrf;
        ierr = MatSetValues(H, 4, row, 1, col, val, ADD_VALUES);
        CHKERRQ(ierr);
        ...
        val[0] = val[1] = val[2] = val[3] = 0.0;
        val[0] =
            lambda[gloc] * dPf_dVrf_dVif + lambda[gloc + 1] * dQf_dVrf_dVif;
        val[1] =
            lambda[gloc] * dPf_dVif_dVif + lambda[gloc + 1] * dQf_dVif_dVif;
        val[2] =
            lambda[gloc] * dPf_dVrt_dVif + lambda[gloc + 1] * dQf_dVrt_dVif;
        val[3] =
            lambda[gloc] * dPf_dVit_dVif + lambda[gloc + 1] * dQf_dVit_dVif;
        ...
        val[0] = val[1] = val[2] = val[3] = 0.0;
        val[0] =
            lambda[gloc] * dPf_dVrf_dVrt + lambda[gloc + 1] * dQf_dVrf_dVrt;
        val[1] =
            lambda[gloc] * dPf_dVif_dVrt + lambda[gloc + 1] * dQf_dVif_dVrt;
        val[2] =
            lambda[gloc] * dPf_dVrt_dVrt + lambda[gloc + 1] * dQf_dVrt_dVrt;
        val[3] =
            lambda[gloc] * dPf_dVit_dVrt + lambda[gloc + 1] * dQf_dVit_dVrt;
        ierr = MatSetValues(H, 4, row, 1, col, val, ADD_VALUES);
        CHKERRQ(ierr);
        ...
        val[0] = val[1] = val[2] = val[3] = 0.0;
        val[0] =
            lambda[gloc] * dPf_dVrf_dVit + lambda[gloc + 1] * dQf_dVrf_dVit;
        val[1] =
            lambda[gloc] * dPf_dVif_dVit + lambda[gloc + 1] * dQf_dVif_dVit;
        val[2] =
            lambda[gloc] * dPf_dVrt_dVit + lambda[gloc + 1] * dQf_dVrt_dVit;
        val[3] =
            lambda[gloc] * dPf_dVit_dVit + lambda[gloc + 1] * dQf_dVit_dVit;
        flps += 92;
        ierr = MatSetValues(H, 4, row, 1, col, val, ADD_VALUES);
        CHKERRQ(ierr);
      } else {
        ...
        val[0] = val[1] = val[2] = val[3] = 0.0;
        val[0] =
            lambda[gloc] * dPt_dVrt_dVrt + lambda[gloc + 1] * dQt_dVrt_dVrt;
        val[1] =
            lambda[gloc] * dPt_dVit_dVrt + lambda[gloc + 1] * dQt_dVit_dVrt;
        val[2] =
            lambda[gloc] * dPt_dVrf_dVrt + lambda[gloc + 1] * dQt_dVrf_dVrt;
        val[3] =
            lambda[gloc] * dPt_dVif_dVrt + lambda[gloc + 1] * dQt_dVif_dVrt;
        ierr = MatSetValues(H, 4, row, 1, col, val, ADD_VALUES);
        CHKERRQ(ierr);
        ...
        val[0] = val[1] = val[2] = val[3] = 0.0;
        val[0] =
            lambda[gloc] * dPt_dVrt_dVit + lambda[gloc + 1] * dQt_dVrt_dVit;
        val[1] =
            lambda[gloc] * dPt_dVit_dVit + lambda[gloc + 1] * dQt_dVit_dVit;
        val[2] =
            lambda[gloc] * dPt_dVrf_dVit + lambda[gloc + 1] * dQt_dVrf_dVit;
        val[3] =
            lambda[gloc] * dPt_dVif_dVit + lambda[gloc + 1] * dQt_dVif_dVit;
        ierr = MatSetValues(H, 4, row, 1, col, val, ADD_VALUES);
        CHKERRQ(ierr);
        ...
        val[0] = val[1] = val[2] = val[3] = 0.0;
        val[0] =
            lambda[gloc] * dPt_dVrt_dVrf + lambda[gloc + 1] * dQt_dVrt_dVrf;
        val[1] =
            lambda[gloc] * dPt_dVit_dVrf + lambda[gloc + 1] * dQt_dVit_dVrf;
        val[2] =
            lambda[gloc] * dPt_dVrf_dVrf + lambda[gloc + 1] * dQt_dVrf_dVrf;
        val[3] =
            lambda[gloc] * dPt_dVif_dVrf + lambda[gloc + 1] * dQt_dVif_dVrf;
        ierr = MatSetValues(H, 4, row, 1, col, val, ADD_VALUES);
        CHKERRQ(ierr);
        ...
        val[0] = val[1] = val[2] = val[3] = 0.0;
        val[0] =
            lambda[gloc] * dPt_dVrt_dVif + lambda[gloc + 1] * dQt_dVrt_dVif;
        val[1] =
            lambda[gloc] * dPt_dVit_dVif + lambda[gloc + 1] * dQt_dVit_dVif;
        val[2] =
            lambda[gloc] * dPt_dVrf_dVif + lambda[gloc + 1] * dQt_dVrf_dVif;
        val[3] =
            lambda[gloc] * dPt_dVif_dVif + lambda[gloc + 1] * dQt_dVif_dVif;
        flps += 92;
        ierr = MatSetValues(H, 4, row, 1, col, val, ADD_VALUES);
        CHKERRQ(ierr);
      }
  ...
  ierr = VecRestoreArrayRead(localX, &x);
  CHKERRQ(ierr);
  ierr = VecRestoreArrayRead(opflow->Lambdaelocal, &lambda);
  CHKERRQ(ierr);
...
*/
PetscErrorCode OPFLOWComputeInequalityConstraintsHessian_PBCAR(OPFLOW opflow,
                                                               Vec X,
                                                               Vec Lambda,
                                                               Mat H) {
  PetscErrorCode ierr;
  ...
  ierr = MatGetOwnershipRange(H, &rstart, NULL);
  CHKERRQ(ierr);
...
*/
PetscErrorCode OPFLOWComputeObjectiveHessian_PBCAR(OPFLOW opflow, Vec X,
                                                   Mat H) {
  PetscErrorCode ierr;
  PS ps = opflow->ps;
  PetscInt i, k;
...
PetscErrorCode OPFLOWComputeHessian_PBCAR(OPFLOW opflow, Vec X, Vec Lambdae,
                                          Vec Lambdai, Mat H) {
  PetscErrorCode ierr;
  PetscFunctionBegin;
  ...
  ierr = OPFLOWComputeObjectiveHessian_PBCAR(opflow, X, H);
  CHKERRQ(ierr);

  /* Equality constraints Hessian */
  ierr = OPFLOWComputeEqualityConstraintsHessian_PBCAR(opflow, X, Lambdae, H);
  CHKERRQ(ierr);

  /* Inequality constraints Hessian */
  if (opflow->nconineq) {
    ierr =
        OPFLOWComputeInequalityConstraintsHessian_PBCAR(opflow, X, Lambdai, H);
    CHKERRQ(ierr);
  }
...
PetscErrorCode OPFLOWSolutionToPS_PBCAR(OPFLOW opflow) {
  ...
  if (opflow->comm->size > 1) {
    SETERRQ(opflow->comm->type, PETSC_ERR_SUP,
            "Parallel solution printing/saving not supported yet\n");
  }
  ...
  for (i = 0; i < ps->nbus; i++) {
    ...
    gloc += 2;
    flps += 3 + EXAGO_FLOPS_TANOP + EXAGO_FLOPS_SQRTOP;
...
PetscErrorCode OPFLOWModelCreate_PBCAR(OPFLOW opflow) {
  ...
  ierr = PetscCalloc1(1, &pbcar);
  CHKERRQ(ierr);

  opflow->model = pbcar;

  /* Inherit Ops */
  opflow->modelops.destroy = OPFLOWModelDestroy_PBCAR;
  opflow->modelops.setnumvariables = OPFLOWModelSetNumVariables_PBCAR;
  opflow->modelops.setnumconstraints = OPFLOWModelSetNumConstraints_PBCAR;
  opflow->modelops.setvariablebounds = OPFLOWSetVariableBounds_PBCAR;
  opflow->modelops.setconstraintbounds = OPFLOWSetConstraintBounds_PBCAR;
  opflow->modelops.setvariableandconstraintbounds =
      OPFLOWSetVariableandConstraintBounds_PBCAR;
  opflow->modelops.setinitialguess = OPFLOWSetInitialGuess_PBCAR;
  ...
  opflow->modelops.computehessian = OPFLOWComputeHessian_PBCAR;
