...
PetscErrorCode OPFLOWModelDestroy_PBPOLUC(OPFLOW opflow) {
  PetscErrorCode ierr;
...
PetscErrorCode OPFLOWSetVariableBounds_PBPOLUC(OPFLOW opflow, Vec Xl, Vec Xu) {
  PetscErrorCode ierr;
  PS ps = opflow->ps;
  PetscScalar *xl, *xu;
  PetscInt i;
  ...
  for (i = 0; i < ps->nbus; i++) {
    ...
    for (k = 0; k < bus->ngen; k++) {
      ...
      /* Pg and Qg are constrained by their bounds in the constraint functions, so we don't need to bound these here
       */
      xl[loc] = PETSC_NINFINITY;
      xu[loc] = PETSC_INFINITY;
      ...
      /* generator status bounds */
      loc = gen->startxstatusloc;
      xl[loc] = xl[loc+1] = 0.0;
      xu[loc] = xu[loc+1] = 1.0;
...
PetscErrorCode OPFLOWSetConstraintBounds_PBPOLUC(OPFLOW opflow, Vec Gl, Vec Gu) {
  PetscErrorCode ierr;
  PS ps = opflow->ps;
  PetscScalar *gl, *gu;
  PetscInt i, k, ngen;
  ...
  for (i = 0; i < ps->nbus; i++) {
    ...
    for (k = 0; k < ngen; k++) {
      ...
      gloc = gen->starteqloc;
      gl[gloc] = gl[gloc+1] = 1.0;
      gu[gloc] = gu[gloc+1] = 1.0;
  ...
  for (i = 0; i < ps->nbus; i++) {
    ...
    for (k = 0; k < bus->ngen; k++) {
      ...
      /* Inequality constraint bounds for generator status */
      gl[gloc] = gl[gloc+1] = gl[gloc+2] = gl[gloc+3] = PETSC_NINFINITY;
...
PetscErrorCode OPFLOWSetVariableandConstraintBounds_PBPOLUC(OPFLOW opflow, Vec Xl,
                                                          Vec Xu, Vec Gl,
                                                          Vec Gu) {
  PetscErrorCode ierr;
  ...
  ierr = OPFLOWSetVariableBounds_PBPOLUC(opflow, Xl, Xu);
  CHKERRQ(ierr);
  ierr = OPFLOWSetConstraintBounds_PBPOLUC(opflow, Gl, Gu);
  CHKERRQ(ierr);

  PetscFunctionReturn(0);
}

PetscErrorCode OPFLOWSetInitialGuess_PBPOLUC(OPFLOW opflow, Vec X, Vec Lambda) {
  PetscErrorCode ierr;
  PS ps = opflow->ps;
...
PetscErrorCode OPFLOWComputeEqualityConstraints_PBPOLUC(OPFLOW opflow, Vec X,
                                                      Vec Ge) {
  PetscErrorCode ierr;
  ...
  for (i = 0; i < ps->nbus; i++) {
    ...
    for (k = 0; k < bus->ngen; k++) {
      ...
      PetscScalar u,z;
      gloc = gen->starteqloc;
      u = x[gen->startxstatusloc];
      z = x[gen->startxstatusloc+1];
...
PetscErrorCode OPFLOWComputeEqualityConstraintJacobian_PBPOLUC(OPFLOW opflow,
                                                             Vec X, Mat Je) {
  PetscErrorCode ierr;
  ...
  for (i = 0; i < ps->nbus; i++) {
    ...
    for (k = 0; k < bus->ngen; k++) {
      ...
      locglob = gen->startxstatuslocglob;
...
PetscErrorCode OPFLOWComputeInequalityConstraints_PBPOLUC(OPFLOW opflow, Vec X,
                                                        Vec Gi) {
  PetscErrorCode ierr;
  PetscInt i, k;
  PetscInt gloc;
  ...
  for (i = 0; i < ps->nbus; i++) {
    ...
    for (k = 0; k < bus->ngen; k++) {
      ...
      /* Inequality constraints for commitment */
      u =  x[gen->startxstatusloc];
      Pg = x[gen->startxpowloc];
      Qg = x[gen->startxpowloc+1];

      g[gloc]   = Pg - u*gen->pt;
      g[gloc+1] = u*gen->pb - Pg;
      g[gloc+2] = Qg - u*gen->qt;
      g[gloc+3] = u*gen->qb - Qg;
      gloc += 4;
...
PetscErrorCode OPFLOWComputeInequalityConstraintJacobian_PBPOLUC(OPFLOW opflow,
                                                               Vec X, Mat Ji) {
  PetscErrorCode ierr;
  PetscInt i, k, flps = 0;
  ...
  for (i = 0; i < ps->nbus; i++) {
    ...
    for (k = 0; k < bus->ngen; k++) {
      ...
      col[1] = gen->startxstatusloc;
      val[0] = 1.0;
      val[1] = -gen->pt;
      val[2] = -1.0;
      val[3] = gen->pb;
      ierr = MatSetValues(Ji,2,row,2,col,val,ADD_VALUES);
      CHKERRQ(ierr);

      row[0] = gloc+2;
      row[1] = gloc+3;
      col[0] = gen->startxpowloc+1;
      col[1] = gen->startxstatusloc;
      val[0] = 1.0;
      val[1] = -gen->qt;
      val[2] = -1.0;
...
PetscErrorCode OPFLOWComputeConstraints_PBPOLUC(OPFLOW opflow, Vec X, Vec G) {
  // empty function
  (void)opflow;
  (void)X;
  (void)G;
...
PetscErrorCode OPFLOWComputeObjective_PBPOLUC(OPFLOW opflow, Vec X,
                                            PetscScalar *obj) {
  PetscErrorCode ierr;
  const PetscScalar *x;
  PS ps = opflow->ps;
  ...
  *obj = 0.0;
  ...
  for (i = 0; i < ps->nbus; i++) {
	...
	u = x[loc];
        ...
        Pg = x[loc] * ps->MVAbase;
        *obj +=
            gen->cost_alpha * Pg * Pg + gen->cost_beta * Pg + u*gen->cost_gamma;
        flps += 7.0;
...
PetscErrorCode OPFLOWComputeGradient_PBPOLUC(OPFLOW opflow, Vec X, Vec grad) {
  PetscErrorCode ierr;
  const PetscScalar *x;
  PetscScalar *df;
  PS ps = opflow->ps;
...
PetscErrorCode OPFLOWComputeObjandGradient_PBPOLUC(OPFLOW opflow, Vec X,
                                                 PetscScalar *obj, Vec Grad) {
  PetscErrorCode ierr;
  PetscFunctionBegin;

  ierr = OPFLOWComputeObjective_PBPOLUC(opflow, X, obj);
  CHKERRQ(ierr);
  ierr = OPFLOWComputeGradient_PBPOLUC(opflow, X, Grad);
  CHKERRQ(ierr);
...
PetscErrorCode OPFLOWModelSetNumVariables_PBPOLUC(OPFLOW opflow,
                                                PetscInt *busnvar,
                                                PetscInt *branchnvar,
                                                PetscInt *nx) {
  PetscInt i, ngen, k;
  ...
  *nx = 0;
  ...
  for (i = 0; i < ps->nbus; i++) {
    ...
    for (k = 0; k < ngen; k++) {
      ...
      gen->nxpow = 2; /* 2 variables - Pg and Qg */
      gen->nxstatus = 2; /* 2 variables - u and z */
      if (opflow->has_gensetpoint && !gen->isrenewable) {
        gen->nxpset = 1;
        gen->nxpdev = 1;
        busnvar[i] += 2;
      }
      gen->nx = gen->nxpow + gen->nxpdev + gen->nxstatus;
      busnvar[i] += gen->nx;
    }
 ...
 * i.e., bus, gen, load, and line */
PetscErrorCode OPFLOWModelSetNumConstraints_PBPOLUC(OPFLOW opflow,
...
*/
PetscErrorCode OPFLOWComputeEqualityConstraintsHessian_PBPOLUC(OPFLOW opflow,
                                                             Vec X, Vec Lambda,
                                                             Mat H) {
  PetscErrorCode ierr;
  PS ps = opflow->ps;
...
*/
PetscErrorCode OPFLOWComputeInequalityConstraintsHessian_PBPOLUC(OPFLOW opflow,
                                                               Vec X,
                                                               Vec Lambda,
                                                               Mat H) {
  PetscErrorCode ierr;
...
*/
PetscErrorCode OPFLOWComputeObjectiveHessian_PBPOLUC(OPFLOW opflow, Vec X,
                                                   Mat H) {
  PetscErrorCode ierr;
  PS ps = opflow->ps;
  PetscInt i, k;
...
PetscErrorCode OPFLOWComputeHessian_PBPOLUC(OPFLOW opflow, Vec X, Vec Lambdae,
                                          Vec Lambdai, Mat H) {
  PetscErrorCode ierr;
  PetscFunctionBegin;
  ...
  ierr = OPFLOWComputeObjectiveHessian_PBPOLUC(opflow, X, H);
  CHKERRQ(ierr);

  /* Equality constraints Hessian */
  ierr = OPFLOWComputeEqualityConstraintsHessian_PBPOLUC(opflow, X, Lambdae, H);
  CHKERRQ(ierr);

  /* Inequality constraints Hessian */
  if (opflow->nconineq) {
    ierr =
        OPFLOWComputeInequalityConstraintsHessian_PBPOLUC(opflow, X, Lambdai, H);
    CHKERRQ(ierr);
  }
...
PetscErrorCode OPFLOWSolutionToPS_PBPOLUC(OPFLOW opflow) {
  PetscErrorCode ierr;
  PS ps = (PS)opflow->ps;
  PetscInt i, k;
  Vec X, Lambda;
  ...
  for (i = 0; i < ps->nbus; i++) {
    ...
    for (k = 0; k < bus->ngen; k++) {
      ...
      if(x[loc] > 0.5) gen->status = PetscCeilReal(x[loc]);
      else gen->status = PetscFloorReal(x[loc]);
      ...
      ierr = PetscPrintf(PETSC_COMM_SELF,"gen %d at bus %d u = %lf, z = %lf, Pg = %lf, Pmin = %lf, Pmax = %lf\n",k,bus->bus_i,x[gen->startxstatusloc],x[gen->startxstatusloc+1],x[loc],gen->pb,gen->pt);
...
PetscErrorCode OPFLOWModelSetUp_PBPOLUC(OPFLOW opflow) {
  PetscErrorCode ierr;
  PS ps = (PS)opflow->ps;
  PSBUS bus;
  PSGEN gen;
  ...
  for (i = 0; i < ps->nbus; i++) {
    ...
    for (k = 0; k < bus->ngen; k++) {
      ...
      gen->startxstatusloc = gen->startxpowloc + gen->nxpow;
      gen->startxstatuslocglob = gen->startxpowlocglob + gen->nxpow;

      if (opflow->has_gensetpoint && !gen->isrenewable) {
        gen->startxpdevloc = gen->startxstatusloc + gen->nxstatus;
        gen->startxpdevlocglob = gen->startxstatuslocglob + gen->nxstatus;
      ...
      gen->nx = gen->nxpow + gen->nxpdev + gen->nxpset + gen->nxstatus;
      loc += gen->nx;
      locglob += gen->nx;
    }
...
PetscErrorCode OPFLOWModelCreate_PBPOLUC(OPFLOW opflow) {
  PBPOLUC pbpoluc;
  PetscErrorCode ierr;

  PetscFunctionBegin;

  ierr = PetscCalloc1(1, &pbpoluc);
  CHKERRQ(ierr);

  opflow->model = pbpoluc;

  /* Inherit Ops */
  opflow->modelops.destroy = OPFLOWModelDestroy_PBPOLUC;
  opflow->modelops.setnumvariables = OPFLOWModelSetNumVariables_PBPOLUC;
  opflow->modelops.setnumconstraints = OPFLOWModelSetNumConstraints_PBPOLUC;
  opflow->modelops.setvariablebounds = OPFLOWSetVariableBounds_PBPOLUC;
  opflow->modelops.setconstraintbounds = OPFLOWSetConstraintBounds_PBPOLUC;
  opflow->modelops.setvariableandconstraintbounds =
      OPFLOWSetVariableandConstraintBounds_PBPOLUC;
  opflow->modelops.setinitialguess = OPFLOWSetInitialGuess_PBPOLUC;
  opflow->modelops.setup = OPFLOWModelSetUp_PBPOLUC;
  opflow->modelops.computeequalityconstraints =
      OPFLOWComputeEqualityConstraints_PBPOLUC;
  opflow->modelops.computeinequalityconstraints =
      OPFLOWComputeInequalityConstraints_PBPOLUC;
  opflow->modelops.computeequalityconstraintjacobian =
      OPFLOWComputeEqualityConstraintJacobian_PBPOLUC;
  opflow->modelops.computeinequalityconstraintjacobian =
      OPFLOWComputeInequalityConstraintJacobian_PBPOLUC;
  opflow->modelops.computehessian = OPFLOWComputeHessian_PBPOLUC;
  opflow->modelops.computeobjandgradient = OPFLOWComputeObjandGradient_PBPOLUC;
  opflow->modelops.computeobjective = OPFLOWComputeObjective_PBPOLUC;
  opflow->modelops.computegradient = OPFLOWComputeGradient_PBPOLUC;
  opflow->modelops.solutiontops = OPFLOWSolutionToPS_PBPOLUC;
