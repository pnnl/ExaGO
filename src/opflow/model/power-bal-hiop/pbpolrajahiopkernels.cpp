...
PetscErrorCode OPFLOWComputeObjectiveArray_PBPOLRAJAHIOP(OPFLOW opflow,const double *x_dev,double *obj)
{
  ...
  if(opflow->include_loadloss_variables) {
    ...
    RAJA::forall< exago_raja_exec >( RAJA::RangeSegment(0, loadparams->nload), 
      RAJA_LAMBDA (RAJA::Index_type i)
      {
        ...
        obj_val_sum += loadloss_penalty_dev_[i]*MVAbase*MVAbase*(Pdloss*Pdloss + Qdloss*Qdloss);
      }
    );
  }
  ...
  if(opflow->include_powerimbalance_variables) {
    double *powerimbalance_penalty_dev_ = busparams->powerimbalance_penalty_dev_;
    RAJA::forall< exago_raja_exec >( RAJA::RangeSegment(0, busparams->nbus), 
      RAJA_LAMBDA (RAJA::Index_type i)
      {
        double Pimb = x_dev[b_xidxpimb[i]];
        double Qimb = x_dev[b_xidxpimb[i]+1];
        obj_val_sum += powerimbalance_penalty_dev_[i]*MVAbase*MVAbase*(Pimb*Pimb + Qimb*Qimb);
      }
    );
  }
  ...
  if(opflow->include_loadloss_variables) {
    ...
    RAJA::forall< exago_raja_exec >( RAJA::RangeSegment(0, loadparams->nload), 
      RAJA_LAMBDA (RAJA::Index_type i)
      {
        double Pdloss = x_dev[l_xidx[i]];
        double Qdloss = x_dev[l_xidx[i]+1];
        grad_dev[l_xidx[i]]   = loadloss_penalty_dev_[i]*MVAbase*MVAbase*2*Pdloss;
        grad_dev[l_xidx[i]+1] = loadloss_penalty_dev_[i]*MVAbase*MVAbase*2*Qdloss;
      }
    );
  }
  ...
  if(opflow->include_powerimbalance_variables) {
    ...
    RAJA::forall< exago_raja_exec >( RAJA::RangeSegment(0, busparams->nbus), 
      RAJA_LAMBDA (RAJA::Index_type i)
      {
        double Pimb = x_dev[b_xidxpimb[i]];
        double Qimb = x_dev[b_xidxpimb[i]+1];
        grad_dev[b_xidxpimb[i]]   = powerimbalance_penalty_dev_[i]*MVAbase*MVAbase*2*Pimb;
        grad_dev[b_xidxpimb[i]+1] = powerimbalance_penalty_dev_[i]*MVAbase*MVAbase*2*Qimb;
      }
    );
  }
  ...
  if(MHSS_dev != NULL) {
    ...
    if(opflow->include_powerimbalance_variables) {
      ...
      RAJA::forall< exago_raja_exec >( RAJA::RangeSegment(0, busparams->nbus), 
        RAJA_LAMBDA (RAJA::Index_type i)
        {
          MHSS_dev[b_hesssp_idx[i]] = obj_factor*2.0*powerimbalance_penalty_dev_[i]*MVAbase*MVAbase; 
          MHSS_dev[b_hesssp_idx[i]+1] = obj_factor*2.0*powerimbalance_penalty_dev_[i]*MVAbase*MVAbase; 
        }
      );
    }
    ...
    if(opflow->include_loadloss_variables) {
      ...
      RAJA::forall< exago_raja_exec >( RAJA::RangeSegment(0, loadparams->nload), 
        RAJA_LAMBDA (RAJA::Index_type i)
        {
          MHSS_dev[l_hesssp_idx[i]] = obj_factor*2.0*loadloss_penalty_dev_[i]*MVAbase*MVAbase;
          MHSS_dev[l_hesssp_idx[i]+1] = obj_factor*2.0*loadloss_penalty_dev_[i]*MVAbase*MVAbase;
        }
      );
    }
  }    
