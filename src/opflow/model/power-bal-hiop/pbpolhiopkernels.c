...
PetscErrorCode OPFLOWComputeEqualityConstraintsArray_PBPOLHIOP(OPFLOW opflow,const double *x, double *ge)
{
  ...
  for(i=0; i < loadparams->nload; i++) {
    if (opflow->include_loadloss_variables) {
      ...
      ge[loadparams->gidx[i]+1] += loadparams->ql[i] - x[loadparams->xidx[i]+1];
    } 
    else {
...
PetscErrorCode OPFLOWComputeObjectiveArray_PBPOLHIOP(OPFLOW opflow,const double *x,double *obj)
{
  ...
  if(opflow->include_powerimbalance_variables) {
    ...
    for(int i=0; i < busparams->nbus; i++) {
      Pimb = x[busparams->xidxpimb[i]];
      Qimb = x[busparams->xidxpimb[i]+1];
      obj_val += busparams->powerimbalance_penalty[i]*ps->MVAbase*ps->MVAbase*(Pimb*Pimb + Qimb*Qimb);
    }
  }
  ...
  if(opflow->include_loadloss_variables) {
    ...
    for(i=0; i < loadparams->nload; i++) {
      ...
      obj_val +=loadparams->loadloss_penalty[i]*ps->MVAbase*ps->MVAbase*(Pdloss*Pdloss + Qdloss*Qdloss);
    }
  }
  *obj = obj_val;
...
PetscErrorCode OPFLOWComputeGradientArray_PBPOLHIOP(OPFLOW opflow,const double *x, double* grad)
{
  ...
  if(opflow->include_powerimbalance_variables) {
    ...
    for(int i=0; i < busparams->nbus; i++){
      Pimb = x[busparams->xidxpimb[i]];
      Qimb = x[busparams->xidxpimb[i]+1];
      grad[busparams->xidxpimb[i]] = busparams->powerimbalance_penalty[i]*ps->MVAbase*ps->MVAbase*2*Pimb;
      grad[busparams->xidxpimb[i]+1] = busparams->powerimbalance_penalty[i]*ps->MVAbase*ps->MVAbase*2*Qimb;
    }
  }
  ...
  if(opflow->include_loadloss_variables) {
    ...
    for(i=0; i < loadparams->nload; i++) {
      Pdloss = x[loadparams->xidx[i]];
      Qdloss = x[loadparams->xidx[i]+1];
      grad[loadparams->xidx[i]] = loadparams->loadloss_penalty[i]*ps->MVAbase*ps->MVAbase*2*Pdloss;
      grad[loadparams->xidx[i]+1] = loadparams->loadloss_penalty[i]*ps->MVAbase*ps->MVAbase*2*Qdloss;
    }
  }
...
PetscErrorCode OPFLOWSetVariableBoundsArray_PBPOLHIOP(OPFLOW opflow,double *xl,double *xu)
{
  ...
  for(i=0; i < busparams->nbus; i++) {
    ...
    if(opflow->include_powerimbalance_variables) {
      xl[busparams->xidxpimb[i]] = xl[busparams->xidxpimb[i]+1] = PETSC_NINFINITY;
      xu[busparams->xidxpimb[i]] = xu[busparams->xidxpimb[i]+1] = PETSC_INFINITY;
    }
  }
...
PetscErrorCode OPFLOWComputeSparseEqualityConstraintJacobian_PBPOLHIOP(OPFLOW opflow,const double* x,int *iJacS, int *jJacS,double *MJacS)
{
  PBPOLHIOP      pbpolhiop=(PBPOLHIOP)opflow->model;
  GENParams     *genparams=&pbpolhiop->genparams;
  LOADParams     *loadparams=&pbpolhiop->loadparams;
  ...
  if(iJacS != NULL && jJacS != NULL) {
	...
	jJacS[genparams->eqjacspgen_idx[i]+1] = genparams->xidx[i]+2; // delPg

	iJacS[genparams->eqjacspgen_idx[i]+2] = genparams->geqidxgen[i];
	jJacS[genparams->eqjacspgen_idx[i]+2] = genparams->xidx[i]+3; // Pgset

	iJacS[genparams->eqjacspgen_idx[i]+3] = genparams->geqidxgen[i]+1;
	jJacS[genparams->eqjacspgen_idx[i]+3] = genparams->xidx[i]+3; // Pgset
...
PetscErrorCode OPFLOWComputeSparseInequalityConstraintJacobian_PBPOLHIOP(OPFLOW opflow,const double* x,int *iJacS, int *jJacS,double *MJacS)
{
  PBPOLHIOP      pbpolhiop=(PBPOLHIOP)opflow->model;
  GENParams     *genparams=&pbpolhiop->genparams;
  LOADParams     *loadparams=&pbpolhiop->loadparams;
...
PetscErrorCode OPFLOWComputeSparseHessian_PBPOLHIOP(OPFLOW opflow,const double *x,const double *lambda,int *iHSS, int *jHSS,double *MHSS)
{
  PBPOLHIOP      pbpolhiop=(PBPOLHIOP)opflow->model;
  PetscErrorCode ierr;
  GENParams     *genparams=&pbpolhiop->genparams;
  ...
  if(MHSS != NULL) {
    ...
    if(opflow->include_powerimbalance_variables) {
      for(i= 0; i < busparams->nbus; i++) {
        loc = busparams->hesssp_idx[i];
        MHSS[loc] = obj_factor*2.0*busparams->powerimbalance_penalty[i]*ps->MVAbase*ps->MVAbase; 
        MHSS[loc+1] = obj_factor*2.0*busparams->powerimbalance_penalty[i]*ps->MVAbase*ps->MVAbase; 
      }
    }
    ...
    if(opflow->include_loadloss_variables) {
      for(i=0; i < loadparams->nload; i++) {
        loc = loadparams->hesssp_idx[i];
        MHSS[loc] = obj_factor*2.0*loadparams->loadloss_penalty[i]*ps->MVAbase*ps->MVAbase;
        MHSS[loc+1] = obj_factor*2.0*loadparams->loadloss_penalty[i]*ps->MVAbase*ps->MVAbase;
      }
    }
...
PetscErrorCode OPFLOWComputeDenseHessian_PBPOLHIOP(OPFLOW opflow,const double *x, const double *lambda,double *HDD)
{
  ...
  for(i=0; i < nxdense*nxdense; i++) HDD[i] = 0.0;

  /* Equality constraint Hessian */
  ierr = OPFLOWComputeDenseEqualityConstraintHessian_PBPOLHIOP(opflow,x,lambda,HDD);CHKERRQ(ierr);

  if(opflow->nconineq) {
    ierr = OPFLOWComputeDenseInequalityConstraintHessian_PBPOLHIOP(opflow,x,lambda+opflow->nconeq,HDD);CHKERRQ(ierr);
  }
