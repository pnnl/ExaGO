...
PetscErrorCode OPFLOWComputeSparseJacobian_PBPOLHIOP(OPFLOW opflow,int *iJacS, int *jJacS,double *MJacS)
{
  PBPOLHIOP      pbpolhiop=(PBPOLHIOP)opflow->model;
  GENParams     *genparams=&pbpolhiop->genparams;
  int            i;
  ...
  if(MJacS != NULL) {
    /* Generator contributions */
    for(i=0; i < genparams->ngenON; i++) {
      MJacS[genparams->jacsp_idx[i]] = -1.0;
      MJacS[genparams->jacsq_idx[i]] = -1.0;
    }
  }    
...
PetscErrorCode OPFLOWComputeSparseHessian_PBPOLHIOP(OPFLOW opflow,const double *x,int *iHSS, int *jHSS,double *MHSS)
{
  PBPOLHIOP      pbpolhiop=(PBPOLHIOP)opflow->model;
  PetscErrorCode ierr;
  GENParams     *genparams=&pbpolhiop->genparams;
  ...
  if(iHSS != NULL && jHSS != NULL) {
    /* Generator contributions */
    for(i=0; i < genparams->ngenON; i++) {
      iHSS[genparams->jacsp_idx[i]] = genparams->xidx[i];
      jHSS[genparams->jacsp_idx[i]] = genparams->xidx[i];
      
      iHSS[genparams->jacsq_idx[i]] = genparams->xidx[i]+1;
      jHSS[genparams->jacsq_idx[i]] = genparams->xidx[i]+1;
