...
static void SortIndexes(const int &n, int *i_dev, int *j_dev,
                        int *idx_perm_dev) {
  auto &resmgr = umpire::ResourceManager::getInstance();
  umpire::Allocator h_allocator_ = resmgr.getAllocator("HOST");
...
static void PrintTriplets(const std::string &title, const int &n, int *iperm,
  ...
  for (int idx = 0; idx < n; ++idx) {
    ...
    if (vtemp != NULL) {
      std::cout << std::setw(12) << std::right << std::scientific
                << std::setprecision(3) << vtemp[idx];
    }
    std::cout << std::endl;
  }
  h_allocator_.deallocate(itemp);
  ...
  if (MJacS_dev == NULL) {
    ...
    if (opflow->Nconineq) {
      ...
      */
      ...
      SortIndexes(opflow->nnz_ineqjacsp, iJacS_dev + opflow->nnz_eqjacsp,
                  jJacS_dev + opflow->nnz_eqjacsp,
                  pbpolrajahiopsparse->idx_jacineq_dev_);

      if (debugmsg)
        PrintTriplets(
            "Nonzero indexes for Inequality Constraint Jacobian (GPU):",
            opflow->nnz_ineqjacsp, pbpolrajahiopsparse->idx_jacineq_dev_,
            iJacS_dev + opflow->nnz_eqjacsp, jJacS_dev + opflow->nnz_eqjacsp,
            NULL);
      ...
      if (oldhostway) {
        ...
        */
        ...
        if (pbpolrajahiopsparse->i_jacineq == NULL) {
          pbpolrajahiopsparse->i_jacineq = (int *)(h_allocator_.allocate(
              opflow->nnz_ineqjacsp * sizeof(int)));
          pbpolrajahiopsparse->j_jacineq = (int *)(h_allocator_.allocate(
              opflow->nnz_ineqjacsp * sizeof(int)));
        ...
        if (debugmsg)
          PrintTriplets("Nonzero indexes for Inequality Constraint Jacobian:",
                        opflow->nnz_ineqjacsp, NULL,
                        iJacS_dev + opflow->nnz_eqjacsp,
                        jJacS_dev + opflow->nnz_eqjacsp, NULL);
    ...
    if (opflow->Nconineq) {
      ...
      if (!opflow->ignore_lineflow_constraints) {
        ...
        RAJA::forall<exago_raja_exec>(
            ...
            RAJA_LAMBDA(RAJA::Index_type i) {
              ...
              dPf_dthetaf =
                  Vmf * Vmt * (-Gft * sin(thetaft) + Bft * cos(thetaft));
              dPf_dVmf = 2 * Gff * Vmf +
                         Vmt * (Gft * cos(thetaft) + Bft * sin(thetaft));
              dPf_dthetat =
                  Vmf * Vmt * (Gft * sin(thetaft) - Bft * cos(thetaft));
              dPf_dVmt = Vmf * (Gft * cos(thetaft) + Bft * sin(thetaft));

              dQf_dthetaf =
                  Vmf * Vmt * (Bft * sin(thetaft) + Gft * cos(thetaft));
              dQf_dVmf = -2 * Bff * Vmf +
              ...
              dPt_dthetat =
                  Vmt * Vmf * (-Gtf * sin(thetatf) + Btf * cos(thetatf));
              dPt_dVmt = 2 * Gtt * Vmt +
                         Vmf * (Gtf * cos(thetatf) + Btf * sin(thetatf));
              dPt_dthetaf =
                  Vmt * Vmf * (Gtf * sin(thetatf) - Btf * cos(thetatf));
              dPt_dVmf = Vmt * (Gtf * cos(thetatf) + Btf * sin(thetatf));

              dQt_dthetat =
                  Vmt * Vmf * (Btf * sin(thetatf) + Gtf * cos(thetatf));
              dQt_dVmt = -2 * Btt * Vmt +
      ...
      if (debugmsg)
        PrintTriplets(
            "Inequality Constraint Jacobian (GPU):", opflow->nnz_ineqjacsp,
            iperm, (iJacS_dev == NULL ? NULL : iJacS_dev + opflow->nnz_eqjacsp),
      ...
      if (oldhostway) {
        ...
        if (debugmsg)
          PrintTriplets(
              "Inequality Constraint Jacobian:", opflow->nnz_ineqjacsp, NULL,
  ...
  if (MJacS_dev == NULL) {
    ...
    if (debugmsg)
      PrintTriplets("Non-zero indexes for Equality Constraint Jacobian (GPU):",
                    opflow->nnz_eqjacsp, pbpolrajahiopsparse->idx_jaceq_dev_,
    ...
    if (oldhostway) {
      ...
      if (pbpolrajahiopsparse->i_jaceq == NULL) {
        ...
        pbpolrajahiopsparse->j_jaceq =
            (int *)(h_allocator_.allocate(opflow->nnz_eqjacsp * sizeof(int)));
        pbpolrajahiopsparse->val_jaceq = (double *)(h_allocator_.allocate(
            opflow->nnz_eqjacsp * sizeof(double)));
      }
    ...
    RAJA::forall<exago_raja_exec>(
        ...
        RAJA_LAMBDA(RAJA::Index_type i) {
          ...
          MJacS_dev[b_jacsp_idx[i] + 1] =
              isisolated[i] * 0.0 + ispvpq[i] * 2 * Vm * gl[i];
          MJacS_dev[b_jacsq_idx[i]] = 0.0;
          MJacS_dev[b_jacsq_idx[i] + 1] =
              isisolated[i] * 1.0 + ispvpq[i] * -2 * Vm * bl[i];
        });
    ...
    if (debugmsg)
      PrintTriplets("Equality Constraint Jacobian (GPU):", opflow->nnz_eqjacsp,
                    iperm, iJacS_dev, jJacS_dev, MJacS_dev);
    ...
    if (oldhostway) {
      ...
      if (debugmsg)
        PrintTriplets("Equality Constraint Jacobian:", opflow->nnz_eqjacsp,
                      NULL, iJacS_dev, jJacS_dev, MJacS_dev);
    }
  }
...
PetscErrorCode OPFLOWComputeSparseHessian_PBPOLRAJAHIOPSPARSE(
  ...
  if (iHSS_dev != NULL && jHSS_dev != NULL) {
    ...
    if (debugmsg)
      std::cout << "Official Hessian nonzero count: " << opflow->nnz_hesssp
                << std::endl;
    ...
    if (debugmsg)
      PrintTriplets("Hessian Indexes (GPU):", opflow->nnz_hesssp,
                    pbpolrajahiopsparse->idx_hess_dev_, iHSS_dev, jHSS_dev,
                    NULL);
    ...
    if (pbpolrajahiopsparse->i_hess == NULL) {
      ...
      pbpolrajahiopsparse->j_hess =
          (int *)(h_allocator_.allocate(opflow->nnz_hesssp * sizeof(int)));
      pbpolrajahiopsparse->val_hess = (double *)(h_allocator_.allocate(
          opflow->nnz_hesssp * sizeof(double)));
    }
    ...
    */
    ...
    if (debugmsg) {
      PrintTriplets("Hessian Indexes:", opflow->nnz_hesssp, NULL, iHSS_dev,
                    jHSS_dev, NULL);
    }
    ...
    RAJA::forall<exago_raja_exec>(
        ...
        RAJA_LAMBDA(RAJA::Index_type i) {
          ...
          RAJA::atomicAdd<exago_raja_atomic>(&MHSS_dev[b_hesssp_idx[i] + 2],
                                             val);
        });
    ...
    RAJA::forall<exago_raja_exec>(
        ...
        RAJA_LAMBDA(RAJA::Index_type i) {
          ...
          double dPf_dthetaf_dthetaf, dPf_dthetaf_dVmf, dPf_dthetaf_dthetat,
              dPf_dthetaf_dVmt;
          double dPf_dVmf_dthetaf, dPf_dVmf_dVmf, dPf_dVmf_dthetat,
              dPf_dVmf_dVmt;
          double dPf_dthetat_dthetaf, dPf_dthetat_dVmf, dPf_dthetat_dthetat,
              dPf_dthetat_dVmt;
          double dPf_dVmt_dthetaf, dPf_dVmt_dVmf, dPf_dVmt_dthetat,
              dPf_dVmt_dVmt;
          ...
          double dQf_dthetaf_dthetaf, dQf_dthetaf_dVmf, dQf_dthetaf_dthetat,
              dQf_dthetaf_dVmt;
          double dQf_dVmf_dthetaf, dQf_dVmf_dVmf, dQf_dVmf_dthetat,
              dQf_dVmf_dVmt;
          double dQf_dthetat_dthetaf, dQf_dthetat_dVmf, dQf_dthetat_dthetat,
              dQf_dthetat_dVmt;
          double dQf_dVmt_dthetaf, dQf_dVmt_dVmf, dQf_dVmt_dthetat,
              dQf_dVmt_dVmt;
          ...
          if (xidxt[i] > xidxf[i]) {
            RAJA::atomicAdd<exago_raja_atomic>(&MHSS_dev[ln_hessp_idx[i] + 0],
                                               val[2]);
            RAJA::atomicAdd<exago_raja_atomic>(&MHSS_dev[ln_hessp_idx[i] + 1],
                                               val[3]);
            RAJA::atomicAdd<exago_raja_atomic>(&MHSS_dev[ln_hessp_idx[i] + 2],
                                               val[6]);
            RAJA::atomicAdd<exago_raja_atomic>(&MHSS_dev[ln_hessp_idx[i] + 3],
                                               val[7]);
          }
          ...
          double dPt_dthetat_dthetat, dPt_dthetat_dVmt, dPt_dthetat_dthetaf,
              dPt_dthetat_dVmf;
          double dPt_dVmt_dthetat, dPt_dVmt_dVmt, dPt_dVmt_dthetaf,
              dPt_dVmt_dVmf;
          double dPt_dthetaf_dthetat, dPt_dthetaf_dVmt, dPt_dthetaf_dthetaf,
              dPt_dthetaf_dVmf;
          double dPt_dVmf_dthetat, dPt_dVmf_dVmt, dPt_dVmf_dthetaf,
              dPt_dVmf_dVmf;
          ...
          double dQt_dthetaf_dthetaf, dQt_dthetaf_dVmf, dQt_dthetaf_dthetat,
              dQt_dthetaf_dVmt;
          double dQt_dVmf_dthetaf, dQt_dVmf_dVmf, dQt_dVmf_dthetat,
              dQt_dVmf_dVmt;
          double dQt_dthetat_dthetaf, dQt_dthetat_dVmf, dQt_dthetat_dthetat,
              dQt_dthetat_dVmt;
          double dQt_dVmt_dthetaf, dQt_dVmt_dVmf, dQt_dVmt_dthetat,
              dQt_dVmt_dVmt;
    ...
    if (!opflow->ignore_lineflow_constraints) {
      ...
      RAJA::forall<exago_raja_exec>(
          ...
          RAJA_LAMBDA(RAJA::Index_type i) {
            ...
            double d2Pf_dthetaf_dthetaf, d2Pf_dthetaf_dVmf,
                d2Pf_dthetaf_dthetat, d2Pf_dthetaf_dVmt;
            double d2Pf_dVmf_dthetaf, d2Pf_dVmf_dVmf, d2Pf_dVmf_dthetat,
                d2Pf_dVmf_dVmt;
            double d2Pf_dthetat_dthetaf, d2Pf_dthetat_dVmf,
                d2Pf_dthetat_dthetat, d2Pf_dthetat_dVmt;
            ...
            d2Pf_dthetaf_dthetaf =
                -Vmf * Vmt * (Gft * cos(thetaft) + Bft * sin(thetaft));
            d2Pf_dthetaf_dVmf =
                Vmt * (-Gft * sin(thetaft) + Bft * cos(thetaft));
            d2Pf_dthetaf_dthetat =
                Vmf * Vmt * (Gft * cos(thetaft) + Bft * sin(thetaft));
            d2Pf_dthetaf_dVmt =
                Vmf * (-Gft * sin(thetaft) + Bft * cos(thetaft));
            ...
            double d2Qf_dthetaf_dthetaf, d2Qf_dthetaf_dVmf,
                d2Qf_dthetaf_dthetat, d2Qf_dthetaf_dVmt;
            double d2Qf_dVmf_dthetaf, d2Qf_dVmf_dVmf, d2Qf_dVmf_dthetat,
                d2Qf_dVmf_dVmt;
            double d2Qf_dthetat_dthetaf, d2Qf_dthetat_dVmf,
                d2Qf_dthetat_dthetat, d2Qf_dthetat_dVmt;
            ...
            /* dQf_dVmf = -2*Bff*Vmf + Vmt*(-Bft*cos(thetaft) +
             * Gft*sin(thetaft));
             */
            d2Qf_dVmf_dthetaf = Vmt * (Bft * sin(thetaft) + Gft * cos(thetaft));
            d2Qf_dVmf_dVmf = -2 * Bff;
            d2Qf_dVmf_dthetat =
                Vmt * (-Bft * sin(thetaft) - Gft * cos(thetaft));
            ...
            d2Qf_dthetat_dthetat =
                Vmf * Vmt * (Bft * cos(thetaft) - Gft * sin(thetaft));
            d2Qf_dthetat_dVmt =
                Vmf * (-Bft * sin(thetaft) - Gft * cos(thetaft));
            ...
            double d2Pt_dthetat_dthetat, d2Pt_dthetat_dVmt,
                d2Pt_dthetat_dthetaf, d2Pt_dthetat_dVmf;
            double d2Pt_dVmt_dthetat, d2Pt_dVmt_dVmt, d2Pt_dVmt_dthetaf,
                d2Pt_dVmt_dVmf;
            double d2Pt_dthetaf_dthetat, d2Pt_dthetaf_dVmt,
                d2Pt_dthetaf_dthetaf, d2Pt_dthetaf_dVmf;
            ...
            d2Pt_dthetat_dthetat =
                Vmf * Vmt * (-Gtf * cos(thetatf) - Btf * sin(thetatf));
            d2Pt_dthetat_dVmt =
                Vmf * (-Gtf * sin(thetatf) + Btf * cos(thetatf));
            d2Pt_dthetat_dthetaf =
                Vmf * Vmt * (Gtf * cos(thetatf) + Btf * sin(thetatf));
            d2Pt_dthetat_dVmf =
                Vmt * (-Gtf * sin(thetatf) + Btf * cos(thetatf));

            /* dPt_Vmt  = 2*Gtt*Vmt + Vmf*(Gtf*cos(thetatf) + Btf*sin(thetatf));
             */
            d2Pt_dVmt_dthetat =
                Vmf * (-Gtf * sin(thetatf) + Bft * cos(thetatf));
            ...
            double d2Qt_dthetaf_dthetaf, d2Qt_dthetaf_dVmf,
                d2Qt_dthetaf_dthetat, d2Qt_dthetaf_dVmt;
            double d2Qt_dVmf_dthetaf, d2Qt_dVmf_dVmf, d2Qt_dVmf_dthetat,
                d2Qt_dVmf_dVmt;
            double d2Qt_dthetat_dthetaf, d2Qt_dthetat_dVmf,
                d2Qt_dthetat_dthetat, d2Qt_dthetat_dVmt;
            ...
            /* dQt_dVmt = -2*Btt*Vmt + Vmf*(-Btf*cos(thetatf) +
             * Gtf*sin(thetatf));
             */
            d2Qt_dVmt_dthetat = Vmf * (Btf * sin(thetatf) + Gtf * cos(thetatf));
            d2Qt_dVmt_dVmt = -2 * Btt;
            d2Qt_dVmt_dthetaf =
                Vmf * (-Btf * sin(thetatf) + Gtf * cos(thetatf));
            ...
            d2Qt_dthetaf_dthetaf =
                Vmf * Vmt * (Btf * cos(thetatf) - Gtf * sin(thetatf));
            d2Qt_dthetaf_dVmf =
                Vmt * (-Btf * sin(thetatf) - Gtf * cos(thetatf));
            ...
            d2Sf2_dthetaf_dthetaf = 2 * dPf_dthetaf * dPf_dthetaf +
                                    dSf2_dPf * d2Pf_dthetaf_dthetaf +
                                    2 * dQf_dthetaf * dQf_dthetaf +
            ...
            d2Sf2_dthetaf_dthetat = 2 * dPf_dthetat * dPf_dthetaf +
                                    dSf2_dPf * d2Pf_dthetaf_dthetat +
                                    2 * dQf_dthetat * dQf_dthetaf +
            ...
            d2St2_dthetaf_dthetaf = 2 * dPt_dthetaf * dPt_dthetaf +
                                    dSt2_dPt * d2Pt_dthetaf_dthetaf +
                                    2 * dQt_dthetaf * dQt_dthetaf +
            ...
            d2St2_dthetaf_dthetat = 2 * dPt_dthetat * dPt_dthetaf +
                                    dSt2_dPt * d2Pt_dthetaf_dthetat +
                                    2 * dQt_dthetat * dQt_dthetaf +
            ...
            RAJA::atomicAdd<exago_raja_atomic>(&MHSS_dev[fbusidx + 0], val[0]);
            RAJA::atomicAdd<exago_raja_atomic>(&MHSS_dev[fbusidx + 1], val[1]);

            if (xidxt[j] > xidxf[j]) {
              RAJA::atomicAdd<exago_raja_atomic>(&MHSS_dev[ln_hessp_idx[j] + 0],
                                                 val[2]);
              RAJA::atomicAdd<exago_raja_atomic>(&MHSS_dev[ln_hessp_idx[j] + 1],
                                                 val[3]);
            }
            ...
            d2Sf2_dVmf_dVmf =
                2 * dPf_dVmf * dPf_dVmf + dSf2_dPf * d2Pf_dVmf_dVmf +
            ...
            d2Sf2_dVmf_dVmt =
                2 * dPf_dVmt * dPf_dVmf + dSf2_dPf * d2Pf_dVmf_dVmt +
            ...
            d2St2_dVmf_dVmf =
                2 * dPt_dVmf * dPt_dVmf + dSt2_dPt * d2Pt_dVmf_dVmf +
            ...
            d2St2_dVmf_dVmt =
                2 * dPt_dVmt * dPt_dVmf + dSt2_dPt * d2Pt_dVmf_dVmt +
            ...
            RAJA::atomicAdd<exago_raja_atomic>(&MHSS_dev[fbusidx + 2], val[1]);

            if (xidxt[j] > xidxf[j]) {
              RAJA::atomicAdd<exago_raja_atomic>(&MHSS_dev[ln_hessp_idx[j] + 2],
                                                 val[2]);
              RAJA::atomicAdd<exago_raja_atomic>(&MHSS_dev[ln_hessp_idx[j] + 3],
                                                 val[3]);
            }

            //    ierr =
            //    MatSetValues(H,1,row,4,col,val,ADD_VALUES);CHKERRQ(ierr);

            double d2Sf2_dthetat_dthetaf, d2Sf2_dthetat_dVmf,
                d2Sf2_dthetat_dthetat, d2Sf2_dthetat_dVmt;
            double d2St2_dthetat_dthetaf, d2St2_dthetat_dVmf,
                d2St2_dthetat_dthetat, d2St2_dthetat_dVmt;

            d2Sf2_dthetat_dthetaf = 2 * dPf_dthetaf * dPf_dthetat +
            ...
            d2Sf2_dthetat_dthetat = 2 * dPf_dthetat * dPf_dthetat +
                                    dSf2_dPf * d2Pf_dthetat_dthetat +
                                    2 * dQf_dthetat * dQf_dthetat +
            ...
            d2St2_dthetat_dthetaf = 2 * dPt_dthetaf * dPt_dthetat +
                                    dSt2_dPt * d2Pt_dthetat_dthetaf +
                                    2 * dQt_dthetaf * dQt_dthetat +
            ...
            d2St2_dthetat_dthetat = 2 * dPt_dthetat * dPt_dthetat +
                                    dSt2_dPt * d2Pt_dthetat_dthetat +
                                    2 * dQt_dthetat * dQt_dthetat +
            ...
            RAJA::atomicAdd<exago_raja_atomic>(&MHSS_dev[tbusidx + 0], val[2]);
            RAJA::atomicAdd<exago_raja_atomic>(&MHSS_dev[tbusidx + 1], val[3]);
            ...
            d2Sf2_dVmt_dVmf =
                2 * dPf_dVmf * dPf_dVmt + dSf2_dPf * d2Pf_dVmt_dVmf +
            ...
            d2Sf2_dVmt_dVmt =
                2 * dPf_dVmt * dPf_dVmt + dSf2_dPf * d2Pf_dVmt_dVmt +
            ...
            d2St2_dVmt_dVmf =
                2 * dPt_dVmf * dPt_dVmt + dSt2_dPt * d2Pt_dVmt_dVmf +
            ...
            d2St2_dVmt_dVmt =
                2 * dPt_dVmt * dPt_dVmt + dSt2_dPt * d2Pt_dVmt_dVmt +
            ...
            RAJA::atomicAdd<exago_raja_atomic>(&MHSS_dev[tbusidx + 2], val[3]);
    ...
    if (debugmsg) {
      PrintTriplets("Hessian Values (GPU):", opflow->nnz_hesssp, NULL, iHSS_dev,
                    jHSS_dev, MHSS_dev);
    }
    ...
    if (debugmsg) {
      PrintTriplets("Hessian Values:", opflow->nnz_hesssp, NULL, iHSS_dev,
                    jHSS_dev, MHSS_dev);
    }
  }
