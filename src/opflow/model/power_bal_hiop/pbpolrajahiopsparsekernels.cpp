  ...
  if (MHSS_dev != NULL) {
    ...
    if (opflow->include_powerimbalance_variables) {
      RAJA::forall<exago_raja_exec>(
          RAJA::RangeSegment(0, busparams->nbus),
          RAJA_LAMBDA(RAJA::Index_type i) {
            MHSS_dev[b_hesssp_idx[i]] =
                obj_factor * 2.0 * busparams->powerimbalance_penalty_dev_[i] *
                ps->MVAbase * ps->MVAbase;
            MHSS_dev[b_hesssp_idx[i] + 1] =
                obj_factor * 2.0 * busparams->powerimbalance_penalty_dev_[i] *
                ps->MVAbase * ps->MVAbase;
          });
    }
    ...
    if (opflow->include_loadloss_variables) {
      ...
      RAJA::forall<exago_raja_exec>(
          RAJA::RangeSegment(0, loadparams->nload),
          RAJA_LAMBDA(RAJA::Index_type i) {
            MHSS_dev[l_hesssp_idx[i]] = obj_factor * 2.0 *
                                        loadparams->loadloss_penalty_dev_[i] *
                                        ps->MVAbase * ps->MVAbase;
            MHSS_dev[l_hesssp_idx[i] + 1] =
                obj_factor * 2.0 * loadparams->loadloss_penalty_dev_[i] *
                ps->MVAbase * ps->MVAbase;
          });
    }
  }
 ...
 * nxdense
 */
PetscErrorCode OPFLOWComputeDenseEqualityConstraintJacobian_PBPOLRAJAHIOPSPARSE(
 ...
 */
PetscErrorCode OPFLOWComputeDenseEqualityConstraintHessian_PBPOLRAJAHIOPSPARSE(
 ...
 */
PetscErrorCode
OPFLOWComputeDenseInequalityConstraintHessian_PBPOLRAJAHIOPSPARSE(
    OPFLOW opflow, const double *x_dev, const double *lambda_dev,
    double *HDD_dev) {
  PbpolModelRajaHiopSparse *pbpolrajahiopsparse =
      reinterpret_cast<PbpolModelRajaHiopSparse *>(opflow->model);
...
PetscErrorCode OPFLOWComputeDenseHessian_PBPOLRAJAHIOPSPARSE(
    OPFLOW opflow, const double *x_dev, const double *lambda_dev,
    double *HDD_dev) {
  PetscErrorCode ierr;
  int nxdense = opflow->nxdense;
  ...
  ierr = OPFLOWComputeDenseEqualityConstraintHessian_PBPOLRAJAHIOPSPARSE(
      opflow, x_dev, lambda_dev, HDD_dev);
  CHKERRQ(ierr);

  if (opflow->nconineq) {
    ierr = OPFLOWComputeDenseInequalityConstraintHessian_PBPOLRAJAHIOPSPARSE(
