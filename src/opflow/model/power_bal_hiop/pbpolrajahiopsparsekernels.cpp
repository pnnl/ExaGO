...
OPFLOWComputeSparseInequalityConstraintJacobian_PBPOLRAJAHIOPSPARSE(
  ...
  if (MJacS_dev == NULL) {
    ...
    if (opflow->Nconineq) {
      ...
      pbpolrajahiopsparse->i_jacineq = (int*)(h_allocator_.allocate(opflow->nnz_ineqjacsp*sizeof(int)));
	...
	CHKERRQ(ierr);
      ...
      resmgr.copy(iJacS_dev+opflow->nnz_eqjacsp,pbpolrajahiopsparse->i_jacineq);
      resmgr.copy(jJacS_dev+opflow->nnz_eqjacsp,pbpolrajahiopsparse->j_jacineq);
    ...
    if (opflow->Nconineq) {
      ...
      resmgr.copy(MJacS_dev+opflow->nnz_eqjacsp,pbpolrajahiopsparse->val_jacineq);
...
OPFLOWComputeSparseEqualityConstraintJacobian_PBPOLRAJAHIOPSPARSE(
  ...
  if (MJacS_dev == NULL) {
    ...
    ierr = (*opflow->modelops.computeequalityconstraintjacobian)(opflow, opflow->X, opflow->Jac_Ge);
    CHKERRQ(ierr);
...
PetscErrorCode OPFLOWComputeSparseHessian_PBPOLRAJAHIOPSPARSE(
  ...
  if (iHSS_dev != NULL && jHSS_dev != NULL) {
    ...
    pbpolrajahiopsparse->i_hess = (int*)(h_allocator_.allocate(opflow->nnz_hesssp*sizeof(int)));
    pbpolrajahiopsparse->j_hess = (int*)(h_allocator_.allocate(opflow->nnz_hesssp*sizeof(int)));
    pbpolrajahiopsparse->val_hess = (double*)(h_allocator_.allocate(opflow->nnz_hesssp*sizeof(double)));
