...
PetscErrorCode OPFLOWComputeInequalityConstraintsHessian_IBCAR(OPFLOW opflow, Vec X, Vec Lambda,Mat H)
{
  ...
  for(i=0; i < ps->Nbranch; i++) {
    ...
    */
    ...
    /* Note: Some of the entries in the Hessian are not entered, for example H(xlocglob,xlogcglobf+1), because IPOPT
...
PetscErrorCode OPFLOWComputeHessian_IBCAR(OPFLOW opflow,Vec X,Vec Lambda,Mat H)
{
  PetscErrorCode ierr;
  PetscFunctionBegin;
  ...
  ierr = OPFLOWComputeEqualityConstraintsHessian_IBCAR(opflow,X,Lambda,H);CHKERRQ(ierr);
  
  /* Inequality constraints Hessian */
  if(opflow->nconineq) {
    ierr = OPFLOWComputeInequalityConstraintsHessian_IBCAR(opflow,X,Lambda,H);CHKERRQ(ierr);
  }
