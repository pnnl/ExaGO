		       ...
		       double* cons)
{
  ...
  /* Equality constaints */
  ierr = VecPlaceArray(opflow->Ge,cons);CHKERRQ(ierr);
  ierr = VecSet(opflow->Ge,0.0);CHKERRQ(ierr);
  ...
  if(opflow->nconineq) {
    ...
    ierr = VecPlaceArray(opflow->Gi,cons+opflow->nconeq);CHKERRQ(ierr);
    ierr = VecSet(opflow->Gi,0.0);CHKERRQ(ierr);
...
void OPFLOWHIOPSPARSEInterface::solution_callback(hiop::hiopSolveStatus status,
...
PetscErrorCode OPFLOWSolverGetConvergenceStatus_HIOPSPARSE(OPFLOW opflow,PetscBool *status)
{
  OPFLOWSolver_HIOPSPARSE hiop = (OPFLOWSolver_HIOPSPARSE)opflow->solver;
...
PetscErrorCode OPFLOWSolverGetObjective_HIOPSPARSE(OPFLOW opflow,PetscReal *obj)
{
  PetscFunctionBegin;
  *obj = opflow->obj;
  PetscFunctionReturn(0);
...
PetscErrorCode OPFLOWSolverGetConstraintMultipliers_HIOPSPARSE(OPFLOW opflow,Vec *Lambda)
{
  PetscFunctionBegin;
...
PetscErrorCode OPFLOWSolverCreate_HIOPSPARSE(OPFLOW opflow)
{
  ...
  opflow->solverops.getconvergencestatus = OPFLOWSolverGetConvergenceStatus_HIOPSPARSE;
  ...
  opflow->solverops.getconstraintmultipliers = OPFLOWSolverGetConstraintMultipliers_HIOPSPARSE;
