 ...
 * Can be easily used with existing IPOPT drivers (to solve problems in HiOP's
 * input), for example Ex2 hiop_interface(7);
 ...
 *
 * An example IPOPT driver that solve HiOP NLPs using Ipopt is available upon
 * request.
 *
 */
...
// TO DO: call eval_cons (and Jacob) separately for Eq and Ineq as per
...
public:
  ...
  virtual bool get_bounds_info(Index n, Number *x_l, Number *x_u, Index m,
    ...
    hiopInterfaceBase::NonlinearityType *types =
        new hiopInterfaceBase::NonlinearityType[n];
  ...
  virtual bool eval_jac_g(Index n, const Number *x, bool new_x, Index m,
    ...
    for (long long i = 0; i < m && bret; i++) {
      ...
      if (values) {
        bret =
            hiopNLP->eval_Jac_cons(nll, mll, onell, &i, x, new_x, constraint);
        if (!bret)
          break;
...
public:
  hiopMDS2IpoptTNLP(hiopInterfaceMDS *hiopNLP_) : hiopNLP(hiopNLP_) {
  ...
  bool get_nlp_info(Index &n, Index &m, Index &nnz_jac_g, Index &nnz_h_lag,
    ...
    if (false == hiopNLP->get_sparse_dense_blocks_info(
                     nx_sparse, nx_dense, nnz_sparse_Jaceq, nnz_sparse_Jacineq,
    ...
    n = (int)nvars;
    m = (int)ncons;
    nnz_h_lag = nnz_sparse_Hess_Lagr_SS;
    assert(nnz_sparse_Hess_Lagr_SD == 0);
    // plus the dense part
  ...
  bool get_bounds_info(Index n, Number *x_l, Number *x_u, Index m, Number *g_l,
    ...
    if (cons_eq_idxs != NULL)
      delete[] cons_eq_idxs;
    if (cons_ineq_idxs != NULL)
      delete[] cons_ineq_idxs;
    ...
    for (int it = 0; it < m; it++) {
      if (g_l[it] == g_u[it])
        cons_eq_idxs[it_eq++] = it;
      else
        cons_ineq_idxs[it_ineq++] = it;
    }
    assert(it_eq == n_eq);
    assert(it_ineq == n_ineq);
  ...
  bool get_starting_point(Index n, bool init_x, Number *x, bool init_z,
                          ...
                          Number *lambda) {
    assert(false == init_z &&
           "primal-dual restart not supported by the addapter");
    assert(false == init_lambda &&
           "primal-dual restart not supported by the addapter");
    long long nll = n;
    return hiopNLP->get_starting_point(nll, x);
  ...
  /** Method to return the constraint residuals */
  // HiOp calls Eq and Ineq separately -> the interface expects that so we have
  // to mimic it
  bool eval_g(Index n, const Number *x, bool new_x, Index m, Number *g) {
    ...
    bool try_onecall_Jac = false;
    {
      ...
      bret =
          hiopNLP->eval_cons(nll, mll, num_cons, cons_eq_idxs, x, new_x, g_eq);
      if (bret) {
    ...
    }
    {
      ...
      bret = hiopNLP->eval_cons(nll, mll, num_cons, cons_ineq_idxs, x, new_x,
                                g_ineq);
      if (bret) {
  ...
  bool eval_jac_g(Index n, const Number *x, bool new_x, Index m, Index nele_jac,
    ...
    if (values == NULL) {
      ...
      {
        long long num_cons = n_eq;
        bret = hiopNLP->eval_Jac_cons(nll, mll, num_cons, cons_eq_idxs, x,
                                      new_x, nx_sparse, nx_dense,
                                      nnz_sparse_Jaceq, iRow, jCol, NULL, NULL);
        if (bret) {
      ...
      {
        long long num_cons = n_ineq;
        bret = hiopNLP->eval_Jac_cons(
            nll, mll, num_cons, cons_ineq_idxs, x, new_x, nx_sparse, nx_dense,
            nnz_sparse_Jacineq, iRow + nnzit, jCol + nnzit, NULL, NULL);
        if (bret) {
      ...
      // avoid unnecessary reallocations when one-call constraints/Jacobian is
      // active
      if (false == onecall_Jac_detected_) {
        ...
        // sparse Jac Eq
        {
          long long num_cons = n_eq;
          bret = hiopNLP->eval_Jac_cons(
              nll, mll, num_cons, cons_eq_idxs, x, new_x, nx_sparse, nx_dense,
              nnz_sparse_Jaceq, NULL, NULL, values, JacDeq->local_data());
          if (bret) {
            nnzit += nnz_sparse_Jaceq;
            assert(nnzit <= nele_jac);
        ...
        {
          long long num_cons = n_ineq;
          bret = hiopNLP->eval_Jac_cons(nll, mll, num_cons, cons_ineq_idxs, x,
                                        new_x, nx_sparse, nx_dense,
                                        nnz_sparse_Jacineq, NULL, NULL,
                                        values + nnzit, JacDineq->local_data());
          if (bret) {
            nnzit += nnz_sparse_Jacineq;
            assert(nnzit <= nele_jac);
            ...
            // the dense part
            memcpy(values + nnzit, JacDineq->local_data(),
                   len * sizeof(double));
      ...
      if (try_onecall_Jac) {
        ...
        bret =
            hiopNLP->eval_Jac_cons(nll, mll, x, new_x, nx_sparse, nx_dense,
                                   nnz_sparse_Jaceq + nnz_sparse_Jacineq, NULL,
                                   NULL, values, JacDeqineq->local_data());
        if (!bret)
          return false;

        int nnzit = nnz_sparse_Jaceq + nnz_sparse_Jacineq;
        // put the dense part of the MDS in the Ipopt sparse Jac matrix
        memcpy(values + nnzit, JacDeqineq->local_data(),
               ((size_t)m * nx_dense) * sizeof(double));
        nnzit += m * nx_dense;
  ...
  bool eval_h(Index n, const Number *x, bool new_x, Number obj_factor, Index m,
    ...
    if (values == NULL) {
      ...
      bret = hiopNLP->eval_Hess_Lagr(
          nll, mll, x, new_x, obj_factor, lambda, new_lambda, nx_sparse,
          nx_dense, nnz_sparse_Hess_Lagr_SS, iRow, jCol, NULL, NULL,
      ...
      bret = hiopNLP->eval_Hess_Lagr(
          nll, mll, x, new_x, obj_factor, lambda, new_lambda, nx_sparse,
          nx_dense, nnz_sparse_Hess_Lagr_SS, NULL, NULL, values, HessMat,
          nnz_sparse_Hess_Lagr_SD, NULL, NULL, NULL);
      if (!bret)
        return false;
      nnzit += nnz_sparse_Hess_Lagr_SS;
  ...
  void finalize_solution(SolverReturn status, Index n, const Number *x,
                         ...
                         Number obj_value, const IpoptData *ip_data,
                         IpoptCalculatedQuantities *ip_cq) {
    //! we use hiop::Solve_Success -> //! TODO: convert between IPOPT and HiOp
    //! err codes
    hiopNLP->solution_callback(hiop::Solve_Success, n, x, z_L, z_U, m, g,
                               lambda, obj_value);
...
private:
  ...
  hiopMatrixDenseRowMajor *JacDeqineq; // this holds the full Jacobian when
                                       // one-call Jacobian is activated
...
public:
  hiopSparse2IpoptTNLP(hiopInterfaceSparse *hiopNLP_)
      : hiopNLP(hiopNLP_), m_nx{0}, m_nnz_sparse_Jaceq{0},
        m_nnz_sparse_Jacineq{0}, m_nnz_sparse_Hess_Lagr{0},
        m_cons_eq_idxs{nullptr}, m_cons_ineq_idxs{nullptr},
        //     m_Jac_eq{nullptr}, m_Jac_ineq{nullptr}, m_Hess{nullptr},
        //     m_Jac{nullptr},
  ...
  bool get_nlp_info(Index &n, Index &m, Index &nnz_jac_g, Index &nnz_h_lag,
    ...
    if (false == hiopNLP->get_sparse_blocks_info(m_nx, m_nnz_sparse_Jaceq,
                                                 m_nnz_sparse_Jacineq,
  ...
  bool get_bounds_info(Index n, Number *x_l, Number *x_u, Index m, Number *g_l,
    ...
    if (m_cons_eq_idxs != NULL)
      delete[] m_cons_eq_idxs;
    if (m_cons_ineq_idxs != NULL)
      delete[] m_cons_ineq_idxs;
    ...
    for (int it = 0; it < m; it++) {
      if (g_l[it] == g_u[it])
        m_cons_eq_idxs[it_eq++] = it;
      else
        m_cons_ineq_idxs[it_ineq++] = it;
    }
    assert(it_eq == m_n_eq);
    assert(it_ineq == m_n_ineq);
  ...
  bool eval_g(Index n, const Number *x, bool new_x, Index m, Number *g) {
...

    {
      ...
      bret = hiopNLP->eval_cons(nll, mll, num_cons, m_cons_eq_idxs, x, new_x,
                                g_eq);
      if (bret) {
...

    {
      ...
      bret = hiopNLP->eval_cons(nll, mll, num_cons, m_cons_ineq_idxs, x, new_x,
                                g_ineq);
      if (bret) {
  ...
  bool eval_jac_g(Index n, const Number *x, bool new_x, Index m, Index nele_jac,
    ...
    if (values == NULL) {
      ...
      // Sparse Jac for Eq
      {
        long long num_cons = m_n_eq;
        bret =
            hiopNLP->eval_Jac_cons(nll, mll, num_cons, m_cons_eq_idxs, x, new_x,
      ...
      {
        long long num_cons = m_n_ineq;
        bret = hiopNLP->eval_Jac_cons(nll, mll, num_cons, m_cons_ineq_idxs, x,
                                      new_x, m_nnz_sparse_Jacineq, iRow + nnzit,
                                      jCol + nnzit, NULL);
        if (bret) {
          // in-place shift of iRow and jCol for Jacineq
      ...
      if (false == m_onecall_Jac_detected_) {
        ...
        {
          long long num_cons = m_n_eq;
          bret = hiopNLP->eval_Jac_cons(nll, mll, num_cons, m_cons_eq_idxs, x,
                                        new_x, m_nnz_sparse_Jaceq, NULL, NULL,
                                        values);
          if (bret) {
            nnzit += m_nnz_sparse_Jaceq;
            assert(nnzit <= nele_jac);
        ...
        // sparse Jac Ineq
        {
          long long num_cons = m_n_ineq;
          bret = hiopNLP->eval_Jac_cons(nll, mll, num_cons, m_cons_ineq_idxs, x,
                                        new_x, m_nnz_sparse_Jacineq, NULL, NULL,
                                        values + nnzit);
          if (bret) {
            nnzit += m_nnz_sparse_Jacineq;
            assert(nnzit <= nele_jac);
  ...
  bool eval_h(Index n, const Number *x, bool new_x, Number obj_factor, Index m,
    ...
    if (values == NULL) {
      ...
      bret = hiopNLP->eval_Hess_Lagr(nll, mll, x, new_x, obj_factor, lambda,
                                     new_lambda, m_nnz_sparse_Hess_Lagr, iRow,
                                     jCol, NULL);
      if (!bret)
        return false;
...
private:
  ...
  //  hiopMatrixSparseTriplet *m_Jac; //this holds the full Jacobian when
  //  one-call Jacobian is activated
