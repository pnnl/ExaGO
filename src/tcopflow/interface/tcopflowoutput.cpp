...
/*
  TCOPFLOWPrintSolution - Prints TCOPFLOW solution to stdout for the given
time-step
...
PetscErrorCode TCOPFLOWPrintSolution(TCOPFLOW tcopflow, PetscInt t_num) {
  ...
  if (!opflow->solutiontops) {
    ierr = TCOPFLOWGetSolution(tcopflow, t_num, &opflow->X);
    CHKERRQ(ierr);
    ierr = TCOPFLOWGetConstraintMultipliers(tcopflow, t_num, &opflow->Lambda);
    CHKERRQ(ierr);
  ...
  CHKERRQ(ierr);
  ierr =
      PetscPrintf(tcopflow->comm->type, "\tMulti-Period Optimal Power Flow\n");
  CHKERRQ(ierr);
  ierr = PetscPrintf(
      tcopflow->comm->type,
      "=============================================================\n");
  ...
  ierr = PetscPrintf(tcopflow->comm->type, "%-35s %s\n", "Solver",
                     tcopflow->solvername);
  CHKERRQ(ierr);
  /*  ierr = PetscPrintf(tcopflow->comm->type,"%-35s
  ...
  ierr = PetscPrintf(tcopflow->comm->type, "%-35s %-5.2f\n",
                     "Duration (minutes)", tcopflow->duration * 60.0);
  CHKERRQ(ierr);
  ierr = PetscPrintf(tcopflow->comm->type, "%-35s %-5.2f\n",
                     "Time-step (minutes)", tcopflow->dT);
  CHKERRQ(ierr);
  ierr = PetscPrintf(tcopflow->comm->type, "%-35s %d\n", "Number of steps",
                     tcopflow->Nt);
  CHKERRQ(ierr);
  ierr = PetscPrintf(
      tcopflow->comm->type, "%-35s %s\n", "Active power demand profile",
      tcopflow->ploadprofileset ? tcopflow->ploadprofile : "NOT SET");
  CHKERRQ(ierr);
  ierr = PetscPrintf(
      tcopflow->comm->type, "%-35s %s\n", "Rective power demand profile",
      tcopflow->qloadprofileset ? tcopflow->qloadprofile : "NOT SET");
  CHKERRQ(ierr);
  ierr = PetscPrintf(
      tcopflow->comm->type, "%-35s %s\n", "Wind generation profile",
      tcopflow->windgenprofileset ? tcopflow->windgenprofile : "NOT SET");
  CHKERRQ(ierr);
  ...
  ierr =
      PetscPrintf(tcopflow->comm->type, "%-35s %s\n", "Power imbalance allowed",
  ...
  CHKERRQ(ierr);
  ierr = PetscPrintf(tcopflow->comm->type, "%-35s %d\n",
                     "Number of equality constraints", tcopflow->Nconeq);
  CHKERRQ(ierr);
  ierr = PetscPrintf(tcopflow->comm->type, "%-35s %d\n",
                     "Number of inequality constraints", tcopflow->Nconineq);
  CHKERRQ(ierr);
  ierr = PetscPrintf(tcopflow->comm->type, "%-35s %d\n",
                     "Number of coupling constraints", tcopflow->Nconcoup);
  CHKERRQ(ierr);
  ...
  CHKERRQ(ierr);
  ierr = TCOPFLOWGetObjective(tcopflow, &cost);
  CHKERRQ(ierr);
...
/*
  TCOPFLOWSaveSolution - Saves the TCOPFLOW solution for the given contingency
to file
...
*/
PetscErrorCode TCOPFLOWSaveSolution(TCOPFLOW tcopflow, PetscInt t_num,
...
*/
PetscErrorCode TCOPFLOWSaveSolutionAll(TCOPFLOW tcopflow, OutputFormat format,
                                       const char outdir[]) {
  PetscErrorCode ierr;
  OPFLOW opflow;
  PetscInt i;
  ...
  for (i = 0; i < tcopflow->Nt; i++) {
    ...
    if (!opflow->solutiontops) {
      ierr = TCOPFLOWGetSolution(tcopflow, i, &opflow->X);
      CHKERRQ(ierr);
      ierr = TCOPFLOWGetConstraintMultipliers(tcopflow, i, &opflow->Lambda);
      CHKERRQ(ierr);
      ierr = OPFLOWSolutionToPS(opflow);
      CHKERRQ(ierr);
    }
    ierr = PetscSNPrintf(filename, 64, "t%d", i);
    CHKERRQ(ierr);
