            ...
            PetscScalar* obj_value, UserDataPtr user_data)
{
  ...
  *obj_value = 0.0;
  ...
  ierr = VecPlaceArray(tcopflow->X,x);CHKERRQ(ierr);
  ierr = (*tcopflow->modelops.computeobjective)(tcopflow,tcopflow->X,obj_value);CHKERRQ(ierr);
                 ...
                 PetscScalar* grad_f, UserDataPtr user_data)
{
  ...
  ierr = VecPlaceArray(tcopflow->X,x);CHKERRQ(ierr);
  ierr = VecPlaceArray(tcopflow->gradobj,grad_f);CHKERRQ(ierr);
            ...
            PetscInt m, PetscScalar* g, UserDataPtr user_data)
{
  ...
  ierr = VecPlaceArray(tcopflow->X,x);CHKERRQ(ierr);
  ierr = VecPlaceArray(tcopflow->G,g);CHKERRQ(ierr);
                ...
                UserDataPtr user_data)
{
  ...
  if(values == NULL) {
    ...
    for(i=0; i < nrow; i++) {
      ierr = MatGetRow(tcopflow->Jac,i,&nvals,&cols,&vals);CHKERRQ(ierr);
      for(j=0; j < nvals; j++) {
	...
	jColstart[j] = coffset + cols[j];
      ...
      iRowstart += nvals;
      jColstart += nvals;
      ierr = MatRestoreRow(tcopflow->Jac,i,&nvals,&cols,&vals);CHKERRQ(ierr);
    }
  } else {
    ierr = VecPlaceArray(tcopflow->X,x);CHKERRQ(ierr);
    /* Compute jacobian */
            ...
            PetscScalar *values, UserDataPtr user_data)
{
  ...
  ierr = MatGetSize(tcopflow->Hes,&nrow,NULL);CHKERRQ(ierr);
  ...
  if(values == NULL) {
    ...
    */
    ...
    /* Copy values */
    ierr = VecPlaceArray(tcopflow->X,x);CHKERRQ(ierr);
    ierr = VecPlaceArray(tcopflow->Lambda,lambda);CHKERRQ(ierr);
...
PetscErrorCode TCOPFLOWSolverSolve_IPOPT(TCOPFLOW tcopflow)
{
  ...
  ierr = (*tcopflow->modelops.computejacobian)(tcopflow,tcopflow->X,tcopflow->Jac);CHKERRQ(ierr);
  ierr = MatSetOption(tcopflow->Jac,MAT_NEW_NONZERO_LOCATION_ERR,PETSC_TRUE);CHKERRQ(ierr);

  ierr = MatGetInfo(tcopflow->Jac,MAT_LOCAL,&info_jac);CHKERRQ(ierr);
  tcopflowipopt->nnz_jac_g = info_jac.nz_used;

  /* Compute non-zeros for Hessian */
  ierr = (*tcopflow->modelops.computehessian)(tcopflow,tcopflow->X,tcopflow->Lambda,tcopflow->Hes);CHKERRQ(ierr);
  ierr = MatSetOption(tcopflow->Hes,MAT_NEW_NONZERO_LOCATION_ERR,PETSC_TRUE);CHKERRQ(ierr);

  ierr  = MatGetInfo(tcopflow->Hes,MAT_LOCAL,&info_hes);CHKERRQ(ierr);
