...
PetscErrorCode TCOPFLOWModelDestroy_GENRAMP(TCOPFLOW tcopflow)
{
  ...
  ierr = PetscFree(tcopflow->model);CHKERRQ(ierr);
  PetscFunctionReturn(0);
}

PetscErrorCode TCOPFLOWSetVariableBounds_GENRAMP(TCOPFLOW tcopflow,Vec Xl,Vec Xu)
{
  PetscErrorCode ierr;
  OPFLOW         opflow;
...
PetscErrorCode TCOPFLOWSetConstraintBounds_GENRAMP(TCOPFLOW tcopflow,Vec Gl,Vec Gu)
{
  PetscInt       i,j,k,ctr;
  PetscErrorCode ierr;
  ...
  for(i=0; i < tcopflow->Nt; i++) {
    if(i > 0) opflowpre = tcopflow->opflows[i-1];
    opflow = tcopflow->opflows[i];
	...
	for(k=0; k < bus->ngen; k++) {
	  ...
	  ierr = PSBUSGetGen(buspre,k,&genpre);CHKERRQ(ierr);
	  if(!gen->status || !genpre->status) continue;
	  /* Ramp constraints */
...
PetscErrorCode TCOPFLOWSetVariableandConstraintBounds_GENRAMP(TCOPFLOW tcopflow,Vec Xl,Vec Xu, Vec Gl, Vec Gu)
{
  PetscErrorCode ierr;

  PetscFunctionBegin;

  ierr = TCOPFLOWSetVariableBounds_GENRAMP(tcopflow,Xl,Xu);CHKERRQ(ierr);
  ierr = TCOPFLOWSetConstraintBounds_GENRAMP(tcopflow,Gl,Gu);CHKERRQ(ierr);
...
PetscErrorCode TCOPFLOWComputeJacobian_GENRAMP(TCOPFLOW tcopflow,Vec X,Mat J)
{
  PetscErrorCode ierr;
...
PetscErrorCode TCOPFLOWComputeConstraints_GENRAMP(TCOPFLOW tcopflow,Vec X,Vec G)
{
  PetscErrorCode ierr;
...
PetscErrorCode TCOPFLOWComputeObjective_GENRAMP(TCOPFLOW tcopflow,Vec X,PetscScalar *obj)
{
  PetscErrorCode ierr;
  OPFLOW         opflow;
  PetscInt       i;
...
PetscErrorCode TCOPFLOWComputeGradient_GENRAMP(TCOPFLOW tcopflow,Vec X,Vec Grad)
{
  PetscErrorCode ierr;
  OPFLOW    opflow;
  PetscInt  i;
...
PetscErrorCode TCOPFLOWComputeObjandGradient_GENRAMP(TCOPFLOW tcopflow,Vec X,PetscScalar *obj,Vec Grad)
{
  PetscErrorCode ierr;
  PetscFunctionBegin;

  ierr = TCOPFLOWComputeObjective_GENRAMP(tcopflow,X,obj);CHKERRQ(ierr);
  ierr = TCOPFLOWComputeGradient_GENRAMP(tcopflow,X,Grad);CHKERRQ(ierr);

  PetscFunctionReturn(0);
}

PetscErrorCode TCOPFLOWModelSetNumVariablesandConstraints_GENRAMP(TCOPFLOW tcopflow,PetscInt *nxi,PetscInt *ngi,PetscInt *nconineqcoup)
{
  PetscInt i,ngenON;
  OPFLOW   opflow;
  PetscErrorCode ierr;
  ...
  for(i=0; i < tcopflow->Nt; i++) {
    ...
    nxi[i] = opflow->nx;
    if(tcopflow->iscoupling) nconineqcoup[i] = (i == 0)?0:ngenON;
    ngi[i] = opflow->ncon + nconineqcoup[i];
  }
  PetscFunctionReturn(0);
}

PetscErrorCode TCOPFLOWComputeHessian_GENRAMP(TCOPFLOW tcopflow,Vec X,Vec Lambda,Mat H)
{
  PetscErrorCode    ierr;
  PetscInt          nrow;
  OPFLOW            opflow;
...
PetscErrorCode TCOPFLOWModelCreate_GENRAMP(TCOPFLOW tcopflow)
{
  ...
  tcopflow->modelops.setnumvariablesandconstraints = TCOPFLOWModelSetNumVariablesandConstraints_GENRAMP;
  ...
  tcopflow->modelops.setvariableandconstraintbounds = TCOPFLOWSetVariableandConstraintBounds_GENRAMP;
  ...
  tcopflow->modelops.computeobjandgradient = TCOPFLOWComputeObjandGradient_GENRAMP;
