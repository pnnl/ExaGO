...
PetscErrorCode SOPFLOWSetVariableBounds_GENRAMP(SOPFLOW sopflow, Vec Xl,
                                                Vec Xu) {
  PetscErrorCode ierr;
  OPFLOW opflow;
...
PetscErrorCode SOPFLOWSetVariableandConstraintBounds_GENRAMP(SOPFLOW sopflow,
                                                             Vec Xl, Vec Xu,
                                                             Vec Gl, Vec Gu) {
  PetscErrorCode ierr;
  ...
  ierr = SOPFLOWSetVariableBounds_GENRAMP(sopflow, Xl, Xu);
  CHKERRQ(ierr);
  ierr = SOPFLOWSetConstraintBounds_GENRAMP(sopflow, Gl, Gu);
  CHKERRQ(ierr);
...
PetscErrorCode SOPFLOWComputeConstraints_GENRAMP(SOPFLOW sopflow, Vec X,
                                                 Vec G) {
  PetscErrorCode ierr;
...
PetscErrorCode SOPFLOWComputeTotalObjective_GENRAMP(SOPFLOW sopflow, Vec X,
                                                    PetscScalar *obj) {
  PetscErrorCode ierr;
  OPFLOW opflow;
  PetscInt i;
  ...
  *obj = 0.0;
  ...
  for (i = 0; i < sopflow->ns; i++) {
    ...
    CHKERRQ(ierr);
    ierr =
        (*opflow->modelops.computeobjective)(opflow, opflow->X, &opflow->obj);
    CHKERRQ(ierr);
    *obj += opflow->obj;
    ierr = VecResetArray(opflow->X);
    CHKERRQ(ierr);
...
PetscErrorCode SOPFLOWComputeBaseObjective_GENRAMP(SOPFLOW sopflow, Vec X,
                                                   PetscScalar *obj) {
  PetscErrorCode ierr;
  OPFLOW opflow;
  PetscInt i;
  ...
  *obj = 0.0;
  ...
  ierr = VecPlaceArray(opflow->X, xi);
  CHKERRQ(ierr);
  ierr = (*opflow->modelops.computeobjective)(opflow, opflow->X, &opflow->obj);
  CHKERRQ(ierr);
  *obj = opflow->obj;
  ierr = VecResetArray(opflow->X);
  CHKERRQ(ierr);
...
PetscErrorCode SOPFLOWComputeGradient_GENRAMP(SOPFLOW sopflow, Vec X,
                                              Vec Grad) {
  PetscErrorCode ierr;
  OPFLOW opflow;
  PetscInt i;
...
PetscErrorCode SOPFLOWModelSetNumVariablesandConstraints_GENRAMP(
...
PetscErrorCode SOPFLOWComputeHessian_GENRAMP(SOPFLOW sopflow, Vec X, Vec Lambda,
                                             Mat H) {
  PetscErrorCode ierr;
  PetscInt nrow;
  OPFLOW opflow;
...
PetscErrorCode SOPFLOWModelCreate_GENRAMP(SOPFLOW sopflow) {
  ...
  sopflow->modelops.setnumvariablesandconstraints =
      SOPFLOWModelSetNumVariablesandConstraints_GENRAMP;
  ...
  sopflow->modelops.setvariableandconstraintbounds =
      SOPFLOWSetVariableandConstraintBounds_GENRAMP;
  ...
  sopflow->modelops.computebaseobjective = SOPFLOWComputeBaseObjective_GENRAMP;
  sopflow->modelops.computetotalobjective =
      SOPFLOWComputeTotalObjective_GENRAMP;
