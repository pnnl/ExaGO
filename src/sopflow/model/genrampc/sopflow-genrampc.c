...
PetscErrorCode SOPFLOWSetVariableBounds_GENRAMPC(SOPFLOW sopflow,Vec Xl,Vec Xu)
{
  PetscErrorCode ierr;
  SCOPFLOW       scopflow;
  OPFLOW         opflow;
  ...
  for(i=0; i < sopflow->ns; i++) {
    ...
    ierr = VecPlaceArray(scopflow->Xl,xli);CHKERRQ(ierr);
    ierr = VecPlaceArray(scopflow->Xu,xui);CHKERRQ(ierr);

    /* Set bounds */
    ierr = (*scopflow->modelops.setvariablebounds)(scopflow,scopflow->Xl,scopflow->Xu);CHKERRQ(ierr);

    ierr = VecResetArray(scopflow->Xl);CHKERRQ(ierr);
    ierr = VecResetArray(scopflow->Xu);CHKERRQ(ierr);
  }
...
PetscErrorCode SOPFLOWSetConstraintBounds_GENRAMPC(SOPFLOW sopflow,Vec Gl,Vec Gu)
{
  PetscInt       i,j,k,ctr;
  PetscErrorCode ierr;
  ...
  for(i=0; i < sopflow->ns; i++) {
    ...
    ierr = VecPlaceArray(scopflow->Gl,gli);CHKERRQ(ierr);
    ierr = VecPlaceArray(scopflow->Gu,gui);CHKERRQ(ierr);

    ierr = (*scopflow->modelops.setconstraintbounds)(scopflow,scopflow->Gl,scopflow->Gu);CHKERRQ(ierr);

    ierr = VecResetArray(scopflow->Gl);CHKERRQ(ierr);
    ierr = VecResetArray(scopflow->Gu);CHKERRQ(ierr);
...
PetscErrorCode SOPFLOWSetVariableandConstraintBounds_GENRAMPC(SOPFLOW sopflow,Vec Xl,Vec Xu, Vec Gl, Vec Gu)
{
  PetscErrorCode ierr;

  PetscFunctionBegin;

  ierr = SOPFLOWSetVariableBounds_GENRAMPC(sopflow,Xl,Xu);CHKERRQ(ierr);
  ierr = SOPFLOWSetConstraintBounds_GENRAMPC(sopflow,Gl,Gu);CHKERRQ(ierr);
...
PetscErrorCode SOPFLOWComputeJacobian_GENRAMPC(SOPFLOW sopflow,Vec X,Mat J)
{
  ...
  for(i=0; i < sopflow->ns; i++) {
    ...
    for(j=0; j < nrow; j++) {
      ierr = MatGetRow(scopflow->Jac,j,&nvals,&cols,&vals);CHKERRQ(ierr);
      for(k=0; k < nvals; k++) {
	row = roffset + j;
	col = coffset + cols[k];
	val = vals[k];
	ierr = MatSetValues(J,1,&row,1,&col,&val,INSERT_VALUES);
      ...
      ierr = MatRestoreRow(scopflow->Jac,j,&nvals,&cols,&vals);CHKERRQ(ierr);
    }
...
PetscErrorCode SOPFLOWComputeConstraints_GENRAMPC(SOPFLOW sopflow,Vec X,Vec G)
{
  PetscErrorCode ierr;
...
PetscErrorCode SOPFLOWComputeObjective_GENRAMPC(SOPFLOW sopflow,Vec X,PetscScalar *obj)
{
  PetscErrorCode ierr;
  SCOPFLOW       scopflow;
  PetscInt       i;
  ...
  *obj = 0.0;
  ...
  for(i=0; i < sopflow->ns; i++) {
    ...
    scopflow = sopflow->scopflows[i];
    ierr = VecPlaceArray(scopflow->X,xi);CHKERRQ(ierr);
    ierr = (*scopflow->modelops.computeobjective)(scopflow,scopflow->X,&scopflowobj);CHKERRQ(ierr);
    *obj += scopflowobj;
    ierr = VecResetArray(scopflow->X);CHKERRQ(ierr);
  }
  ierr = VecRestoreArray(X,&x);CHKERRQ(ierr);
...
PetscErrorCode SOPFLOWComputeGradient_GENRAMPC(SOPFLOW sopflow,Vec X,Vec Grad)
{
  PetscErrorCode ierr;
  SCOPFLOW         scopflow;
  PetscInt       i;
  ...
  for(i=0; i < sopflow->ns; i++) {
    ...
    ierr = VecPlaceArray(scopflow->X,xi);CHKERRQ(ierr);
    ierr = VecPlaceArray(scopflow->gradobj,gradi);CHKERRQ(ierr);
...
PetscErrorCode SOPFLOWComputeObjandGradient_GENRAMPC(SOPFLOW sopflow,Vec X,PetscScalar *obj,Vec Grad)
{
  PetscErrorCode ierr;
  PetscFunctionBegin;

  ierr = SOPFLOWComputeObjective_GENRAMPC(sopflow,X,obj);CHKERRQ(ierr);
  ierr = SOPFLOWComputeGradient_GENRAMPC(sopflow,X,Grad);CHKERRQ(ierr);
...
PetscErrorCode SOPFLOWModelSetNumVariablesandConstraints_GENRAMPC(SOPFLOW sopflow,PetscInt *nxi,PetscInt *ngi,PetscInt *nconineqcoup)
{
  ...
  for(s=0; s < sopflow->ns; s++) {
    ...
    ierr = SCOPFLOWGetNumVariablesandConstraints(scopflow,&nxi[s],&ngi[s]);CHKERRQ(ierr);;
    if(sopflow->iscoupling) nconineqcoup[s] = (s == 0)?0:ngenON;
    ngi[s] += nconineqcoup[s];
  }
  PetscFunctionReturn(0);
}

PetscErrorCode SOPFLOWComputeHessian_GENRAMPC(SOPFLOW sopflow,Vec X,Vec Lambda,Mat H)
{
  PetscErrorCode    ierr;
  PetscInt          nrow;
  SCOPFLOW          scopflow;
  ...
  for(i=0; i < sopflow->ns; i++) {
    ...
    ierr = VecPlaceArray(scopflow->X,xi);CHKERRQ(ierr);
    ierr = VecPlaceArray(scopflow->Lambda,lambdai);CHKERRQ(ierr);
...
PetscErrorCode SOPFLOWModelCreate_GENRAMPC(SOPFLOW sopflow)
{
  ...
  ierr = PetscCalloc1(1,&genrampc);CHKERRQ(ierr);

  sopflow->model = genrampc;

  /* Inherit Ops */
  sopflow->modelops.destroy = SOPFLOWModelDestroy_GENRAMPC;
  sopflow->modelops.setnumvariablesandconstraints = SOPFLOWModelSetNumVariablesandConstraints_GENRAMPC;
  ...
  sopflow->modelops.setvariableandconstraintbounds = SOPFLOWSetVariableandConstraintBounds_GENRAMPC;
  ...
  sopflow->modelops.computeobjandgradient = SOPFLOWComputeObjandGradient_GENRAMPC;
