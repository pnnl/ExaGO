...
\end{tikzpicture}
\caption{Stochastic multi-period contingency constrained example with two scenarios $s_0$ and $s_1$. Each scenario has two contingencies $c_0$ and $c_1$ and each contingency consists of four time-periods $t_0$, $t_1$, $t_2$, $t_3$.  State $s_0,c_0,t_0$ represents the base case (no contingency) case for the two scenarios.  We assume that any contingency is incident at the first time-step, i.e., at $t_0$. Thus, the contingency states $c_1,t_0$ is coupled with the no-contingency state $c_0,t_0$ at time $t_0$ for both the scenarios. The {\textcolor{red}{red}} line denotes the coupling between the contingency and the no-contingency states.The {\textcolor{blue}{blue}} line denotes the coupling between the scenarios}
\label{fig:sctopflow}
\end{figure}
